...
**`BLITZY_RUN_ID` validation constraints.** Operator-supplied values for `BLITZY_RUN_ID` are validated against the regex `^[A-Za-z0-9._-]{1,64}$` and against a small reserved-name set before any filesystem operation. Values containing path separators (`/`, `\`), embedded traversal sequences such as `evil/path/..`, colons (`:`), tildes (`~`), whitespace, control characters, or any character outside `A–Z`, `a–z`, `0–9`, `.`, `_`, `-` are rejected with a structured `ValueError`. Values longer than 64 characters are also rejected. The literal single-component strings `"."` and `".."` are additionally rejected by a defense-in-depth post-check, because the regex's character class includes `.` (required for UUIDv4 fragments and semver-style labels) and those pure-dot strings would otherwise resolve, through `Path(LOGS_DIR / "..")`, to `LOGS_DIR.parent` — a one-level escape from the intended `logs/<run_id>/` scope. The rejection message names the offending environment variable, the observed length, the maximum allowed length (64), the allowed character class, and the validation regex pattern for character-class failures; reserved-name rejections instead name the specific filesystem-reserved category (`"."` or `".."`) and explain why the value would resolve outside `logs/<run_id>/`. Neither error path echoes the offending value itself (the anti-log-injection control documented in `decision-log.md` Row 23). The UUIDv4 default produced when `BLITZY_RUN_ID` is unset or empty satisfies the regex by construction (UUIDv4 strings consist only of `[0-9a-f-]` characters and are 36 characters long) and is never a member of the reserved set. External orchestrators (CI/CD pipelines, manual operators) should configure `BLITZY_RUN_ID` values within the constraint from the outset; typical safe values include UUIDs (`b2c8a0c2-7c4a-4d65-9d1e-1a5f9c8e3b62`), GitHub Actions run identifiers (`12345-67890`), or operator labels (`qa-checkpoint-B-1`).
...
The following nine pitfalls have been encountered during harness development and are documented so that future operators can avoid them.
...
8. **`render_diagrams.py` in containerized environments (Puppeteer/Chrome sandbox).** The default invocation `python3 blitzy/reports/acceleration/scripts/render_diagrams.py` attempts to run the Mermaid CLI (`mmdc`) as the primary validator and falls back to the regex validator if the CLI is unavailable. In containerized environments where the harness runs as `root` (typical of Docker/Kubernetes runners), Chrome's zygote sandbox refuses to launch without explicit `--no-sandbox` flags; the harness now passes `--no-sandbox`, `--disable-setuid-sandbox`, and `--disable-dev-shm-usage` to Chrome via a per-process Puppeteer config file (`--puppeteerConfigFile`) so the CLI can complete the render. If the Puppeteer launch still fails (for example, because Chrome itself is not installed in the container or shared libraries like `libnss3.so` are missing), the harness detects the infrastructure failure via the `_INFRASTRUCTURE_FAILURE_SIGNATURES` substring-match table, logs `failure_class: "infrastructure"` and `fallback_validator: "regex"` in the structured JSON log, and falls back to the regex validator automatically — the script exits 0 for syntactically valid Mermaid blocks regardless of CLI availability. For environments where the operator prefers the regex validator regardless of CLI availability, invoke `python3 blitzy/reports/acceleration/scripts/render_diagrams.py --no-cli` to bypass the Puppeteer probe entirely. The infrastructure-vs-syntax distinction is documented in `decision-log.md` Row 24.
9. **`BLITZY_RUN_ID` value constraints.** Operator-supplied values must match the regex `^[A-Za-z0-9._-]{1,64}$` AND must not be the single-component filesystem-reserved strings `"."` or `".."`. Path separators (`/`, `\`), traversal sequences embedded within longer paths (`evil/path/..`), colons (`:`), and whitespace are blocked by the character class; pure-dot strings are blocked by a defense-in-depth post-check because the regex itself admits the literal `.` (necessary for UUIDv4 fragments and semver-style labels) and `..` alone would otherwise resolve, through `Path(LOGS_DIR / "..")`, to one directory above the intended `logs/<run_id>/` scope. Values longer than 64 characters are also rejected before any filesystem operation. The rejection produces a structured `ValueError` naming the variable, observed length, maximum length, allowed character class, and validation regex pattern (or, for reserved-name failures, the specific reserved category) — but deliberately not the offending value itself, to prevent log injection through the error-handling path. If `BLITZY_RUN_ID` is unset or empty after `strip()`, the harness falls back to an auto-generated UUIDv4 which satisfies both validation stages by construction. The constraint is documented inline in the Environment Variables Reference table above and in `decision-log.md` Row 23.
