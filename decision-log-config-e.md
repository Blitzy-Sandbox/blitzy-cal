...
| **14.** Empty-result handling | Omit the output file entirely, or emit `null`, or emit a JSON object `{}` with a metadata header, or write a literal two-byte string `[]` with no trailing newline. | Write the JSON body `[]` followed by a single trailing `\n` (three bytes total) so the empty case satisfies the literal Directive 3 verification command `cat findings-config-e.json \| wc -l == 1` identically to the non-empty case. | Directive 3 states verbatim "If zero findings, write `[]`" — the JSON body is the empty-array literal `[]`. The Directive 3 verification command `wc -l == 1` is the explicit pass/fail gate, and POSIX `wc -l` counts only newline-terminated lines, so the trailing `\n` is mandatory for the gate to pass. Writing `[]\n` keeps the JSON body equal to `[]` while also satisfying the line-count gate; consumers can still iterate over the parsed empty array without special-casing. | The empty file is three bytes rather than two. Mitigated because every Directive 3 structural requirement (valid JSON, single line of JSON content, identical contract whether empty or populated) is preserved, and the deviation from the AAP's "literal `[]` (two bytes)" narrative is logged in decision 31. |
| **15.** Single-line guarantee | Pretty-print with 2-space indentation for human readability, or emit a stream of newline-delimited JSON (NDJSON) so each finding is on its own line. | Serialize via `JSON.stringify(findings)` with no `space` argument and write `payload + "\n"` so the file is exactly one POSIX line: the JSON body on a single physical line followed by one trailing line terminator. No internal newlines anywhere inside the JSON body. | Directive 3 requires "valid JSON minified to a single line" — the JSON body itself must contain no internal newlines, satisfied by `JSON.stringify(findings)` with no `space` argument. Directive 3 also requires `cat findings-config-e.json \| wc -l == 1` as the explicit pass/fail gate, which under POSIX `wc -l` semantics counts only newline-terminated lines and so mandates exactly one trailing `\n`. NDJSON would be one logical record per line and pretty-printing would emit multiple physical lines — both would fail the single-line clause. | The AAP narrative in §0.5.3 / §0.8.3 describes the implementation as "without a trailing newline" which is internally inconsistent with its own `wc -l == 1` clause. The deviation from that narrative is logged in decision 31. Human readability of the raw findings JSON is unchanged because tools like `jq` can pretty-print on demand without altering the canonical file. |
...
| **31.** `wc -l == 1` vs no-trailing-newline reconciliation | (a) Append a single trailing `\n` so `cat findings-config-e.json \| wc -l` returns `1` (the literal Directive 3 verification command), accepting that the empty case becomes `[]\n` (3 bytes, not 2) and the file ends with `0x0a` (not `]`). (b) Omit the trailing newline so the empty case is exactly `[]` (2 bytes) and the file ends with `]` (`0x5D`), accepting that POSIX `wc -l` reports `0` rather than `1`. | Option (a) — append exactly one trailing `\n`. The JSON body remains on a single physical line; the trailing newline is the line terminator that POSIX `wc -l` counts. Non-empty file: `…]` + `\n` (3,239 bytes for the current 19-finding run). Empty payload: `[]\n` (3 bytes). | The user prompt's CRITICAL Directive 3 specifies an explicit, deterministic acceptance test: `cat findings-config-e.json \| wc -l` must return `1`. Under POSIX `wc -l` semantics this requires exactly one newline character — the canonical line terminator. The user's verbatim verification command is the source of truth; the AAP §0.5.3 / §0.8.3 narrative that says "without a trailing newline" is internally inconsistent with its own `wc -l == 1` clause and is therefore a description of the implementation that cannot satisfy its own gate. Option (a) honours the user's literal acceptance test, keeps the JSON body on a single physical line (decision 15), keeps the JSON content of the empty case equal to `[]` (decision 14), and preserves every other Directive 3 structural requirement (valid JSON, 5-field schema, ≤200-char descriptions, repo-relative paths, UTF-8). | Anyone consuming the file under the (mistaken) expectation that it is "raw JSON with no terminator" sees an extra `0x0a` at end-of-file. Mitigated because (1) the trailing `\n` is the POSIX-standard line terminator that virtually every shell tool, text editor, and JSON parser handles transparently, (2) `JSON.parse(fs.readFileSync(...))` ignores trailing whitespace and returns the same parsed value, and (3) this explicit decision row plus the updated Step 8 verification (which now runs `wc -l == 1` directly per Directive 3) document the deviation from the AAP narrative. |
...
| Normalized output | `findings-config-e.json` (3,239 bytes; minified single-line JSON body + one trailing newline terminator; final byte is `\n` `0x0A`, byte preceding the terminator is `]` `0x5D`) |
...
| Directive 3 pass/fail gates | All gates passing — `cat findings-config-e.json \| wc -l` returns `1` ✓, valid JSON ✓, every finding has all 5 fields ✓, no description exceeds 200 characters ✓ (max observed = 84 chars), UTF-8 ✓, repo-relative paths ✓, single trailing newline ✓ (final byte is `0x0A`; byte preceding is `]` `0x5D`), no internal newlines inside the JSON body ✓, empty-case payload is `[]\n` (3 bytes) but unused for this run because `findings.length === 19 > 0`. The byte-level structural sanity checks are retained in Step 8 of the Re-Run Instructions below as redundant cross-checks alongside the literal Directive 3 `wc -l` command. |
...
// One trailing "\n" so the Directive 3 acceptance test
//   cat findings-config-e.json | wc -l
// returns 1 (POSIX wc -l counts newline-terminated lines). The JSON payload itself
// remains on a single physical line; the trailing newline is the line terminator.
...
The Directive 3 acceptance command is run verbatim first; the remaining commands are redundant byte-level structural cross-checks.

```bash
# Directive 3 verbatim acceptance command: must print 1.
test "$(cat findings-config-e.json | wc -l)" = "1" \
  && echo "wc -l == 1 OK" \
  || (echo "wc -l FAIL" && exit 1)

# Single trailing newline: final byte must be '\n' (0x0A).
test "$(tail -c 1 findings-config-e.json | od -An -tx1 | tr -d ' \n')" = "0a" \
  && echo "trailing newline OK (last byte is '\\n')" \
  || (echo "trailing newline FAIL" && exit 1)

# JSON body on a single physical line: exactly one '\n' byte in the file.
test "$(tr -d -c '\n' < findings-config-e.json | wc -c)" = "1" \
  && echo "single-line JSON body OK" \
...
# 19-finding run this assertion is skipped, but the contract is fixed at '[]\n' = 3 bytes).
node -e "const c=require('fs').readFileSync('findings-config-e.json','utf8');if(c==='[]\n'){console.log('empty case literal []\\\\n OK (3 bytes)');}else{const arr=JSON.parse(c);if(!arr.length)throw new Error('empty array but body is not literal []\\\\n: '+JSON.stringify(c));console.log('non-empty case ('+arr.length+' findings)');}"
```
