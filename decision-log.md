...
|----|-----------|-------------------------|---------------------|------------|
| V1 | **`--dryrun` instead of `--dry-run`** for the Semgrep config check | Prompt text implies `--dry-run` | Modern Semgrep spells the flag `--dryrun` (not `--dry-run`). Per Semgrep's own `--help`, `--dryrun` only suppresses *writing* autofixes to disk and applies solely alongside `--autofix`; without `--autofix` it has no effect and a normal scan still runs — it is not a dedicated offline/no-scan config-only mode. Its config-validation value comes instead from the fact that the three named rule packs must resolve and load for the invocation to run at all, so the `--dryrun` run serves as a config-resolution smoke test that exits `0` only when every pack resolves (served from the local rule cache once populated) and the scan executes without fatal config/pattern errors | Rule-pack resolution and loading are validated and the flag spelling is corrected to the supported form; no effect on findings — the authoritative Layer-2 evidence is the full `--sarif` scan, not the `--dryrun` check |
