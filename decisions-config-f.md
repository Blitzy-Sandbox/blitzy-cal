...
This document satisfies the Explainability rule (AAP §0.7.1) for Config F of the multi-config security-tool comparison. It is the **single source of truth** for every non-trivial implementation decision made while installing OSV-Scanner, scanning the Cal.com (`blitzy-cal`) monorepo lockfile, normalizing findings into the fixed five-field schema, and producing the companion reveal.js executive presentation. It accompanies three sibling deliverables at the repository root: `findings-config-f.json` (primary, minified single-line JSON), `results-osv.json` (raw OSV-Scanner output retained for traceability), and `executive-summary-config-f.html` (reveal.js executive presentation). Rationale for every non-trivial scanning, normalization, and presentation choice is captured in the **Decision Table** below; the sibling deliverable files (`findings-config-f.json`, `results-osv.json`, `executive-summary-config-f.html`) carry only terse structural labels and contain no rationale-bearing comments.
...
| Install method | Prebuilt `linux_amd64` static binary from `github.com/google/osv-scanner/releases/download/v2.3.8/osv-scanner_linux_amd64` placed at `/usr/local/bin/osv-scanner` (58,335,394 bytes; binary mtime `2026-05-15 02:37:57 UTC`; `file(1)` reports `ELF 64-bit LSB executable, x86-64, statically linked, Go BuildID=Hay89NmVN01CyvuvD2zo/oZddOxx_KzwxQQFTDq-M/J5DGzRmWIGm0z7lAAWDJ/npfiHr8w3YHMH129gBso`) |
| Install commands attempted, in literal execution order | (1) `apt-cache policy osv-scanner` → **returned empty** (no candidate in Ubuntu 25.10 repos), so `apt-get install -y osv-scanner` would have failed; (2) **PASS** — downloaded and installed the prebuilt `linux_amd64` binary with the following reproducible command sequence (executed as the root user in the sandbox; substitute `sudo` for non-root environments): `curl -fsSL -o /tmp/osv-scanner_v2.3.8 https://github.com/google/osv-scanner/releases/download/v2.3.8/osv-scanner_linux_amd64` followed by `install -m 0755 /tmp/osv-scanner_v2.3.8 /usr/local/bin/osv-scanner` and finally `rm /tmp/osv-scanner_v2.3.8`. The OSV-Scanner releases page publishes a single `osv-scanner_linux_amd64` binary asset per platform (no archive wrapper), so no `tar`/`unzip` extraction step is required. Verified with `osv-scanner --version` which returned `osv-scanner version: 2.3.8` (see **Install verification (literal stdout)** below). The canonical equivalent using `wget` instead of `curl` is `wget -qO /usr/local/bin/osv-scanner https://github.com/google/osv-scanner/releases/download/v2.3.8/osv-scanner_linux_amd64 && chmod 0755 /usr/local/bin/osv-scanner`. |
...
| Raw vulnerability records returned | `228` findings, spanning `88` distinct package-version entries (`(name, version)` pairs) that resolve to `57` distinct package names (some package names appear at multiple versions in `yarn.lock`). Verified with `jq '[.results[].packages[]? \| {n: .package.name, v: .package.version}] \| unique \| length' < results-osv.json` (returns `88`) and `jq '[.results[].packages[]? \| .package.name] \| unique \| length' < results-osv.json` (returns `57`). |
...
| **Presentation theme inlining.** The full Blitzy reveal.js theme (every CSS custom property and every slide-type / component class enumerated in AAP §0.7.2) is embedded inline in a `<style>` block in `executive-summary-config-f.html`. | (a) Import an external theme stylesheet at `blitzy-deck/references/blitzy-reveal-theme.css`; (b) host the theme as a sibling artifact at the repository root and `<link>` to it. | The canonical theme file `blitzy-deck/references/blitzy-reveal-theme.css` referenced in the Executive Presentation rule does NOT exist in this repository (verified by filesystem search during AAP §0.2.2 context discovery, recorded in AAP §0.7.2). The rule explicitly states "The downstream agent must therefore embed the complete theme inline rather than relying on an external import." | Maintenance: future Blitzy brand changes require updating each config's HTML separately. Acceptable for this comparison artifact; not maintenance-critical because each config is a one-shot deliverable. |
