...
|--------|-------------|----------|--------|----------------|-------------------|----------------|
| **AT-001** | Cal.com lacks a named "Group Admin" role equivalent in the UI | **Low** | ⏳ Partially addressed (via AG-001) | `packages/features/pbac/` | Calendly's 5-role model with Group Admin | PBAC model behaviorally aligned with Calendly's admin/owner/user role structure via `OrganizationPermissionService`. Custom PBAC role template for "Group Admin" deferred to future work item AT-FW-001 (see `specs/admin-teams/future-work.md`). Implementation reference: `packages/features/ee/organizations/lib/` |
| **AT-002** | Cal.com lacks a named "Team Manager" role in the UI | **Low** | ⏳ Partially addressed (via AG-001) | `packages/features/pbac/` | Calendly's Team Manager role | PBAC model behaviorally supports Team Manager equivalent permissions. Custom PBAC role template for "Team Manager" deferred to future work item AT-FW-002 (see `specs/admin-teams/future-work.md`). Implementation reference: `packages/features/ee/organizations/lib/` |
...
**AG-001 — Admin Role Model Parity** (addresses AT-001, AT-002 | validates AG-VAL-001, AG-VAL-006):
- Cal.com's PBAC model behaviorally aligned with Calendly's admin/owner/user role structure
- `OrganizationPermissionService` extended to map Calendly's 5 fixed roles to PBAC permission equivalents
- PBAC-to-Calendly role mapping documented (custom role templates deferred to AT-FW-001/AT-FW-002 per ADR-001)
...
**AG-002 — Team Event Routing Behavioral Parity** (validates AG-VAL-002, AG-VAL-003):
- Round-robin and collective scheduling behaviors aligned with Calendly's team event distribution patterns
- Team creation, configuration, and event routing enhanced for priority-based round-robin and collective multi-host meetings
- Implementation references:
...
**AG-003 — Managed Event Type Push Behavior Parity** (validates AG-VAL-004):
...
**AG-004 — Member Invitation Workflow Parity** (validates AG-VAL-005):
...
<Steps>
  <Step title="Add Default Role Templates — ⏳ Deferred to AT-FW-001/AT-FW-002">
    Pre-configured PBAC role templates for "Group Admin" and "Team Manager" are planned as future work items (AT-FW-001, AT-FW-002). AG-001 addresses behavioral parity by mapping Calendly's role semantics to PBAC permission equivalents in the `OrganizationPermissionService`, but named template creation is deferred per ADR-001.
    
    `Target: packages/features/ee/organizations/lib/` (OrganizationPermissionService — future template additions)
  </Step>
...
The following behavioral acceptance criteria define what constitutes admin governance parity with Calendly. Given Cal.com's existing capabilities, most criteria are **already met or exceeded**. Sprint 7 epics AG-001 through AG-004 address the remaining gaps, targeting validation criteria AG-VAL-001 through AG-VAL-008 as defined in [Validation Criteria](/sprint-roadmap/validation-criteria). Note: AG-VAL-007 (org branding) and AG-VAL-008 (sub-team hierarchies) are existing Cal.com advantages already preserved.
...
### Role and Permission Management (AG-VAL-001, AG-VAL-006)
...
- [ ] Default "Group Admin" and "Team Manager" role templates available — ⏳ Deferred to AT-FW-001/AT-FW-002 (AG-001 addresses behavioral parity via PBAC permission mapping)

### Team Management (AG-VAL-002, AG-VAL-003)

- [x] Teams can be created within organizations
- [x] Sub-teams can be created within teams
- [x] Team event types support Round Robin, Collective, and Managed scheduling — AG-002 team event routing parity targeted
- [x] Team deletion cascades to managed events and memberships
- [x] Team-level routing is supported

### Managed Event Types (AG-VAL-004)

- [x] Admins can create managed event type templates
- [x] Managed events are pushed to team members — AG-003 push behavior targets Calendly's admin-templated distribution
- [x] Managed event settings can be locked by admins (via `SchedulingType.MANAGED`)

### Member Invitation Lifecycle (AG-VAL-005)

- [x] Token-based invitation flow supports batch operations and seat accounting — AG-004 lifecycle parity targeted
