...
| **Role model** | 5 fixed roles: Owner, Admin, Group Admin, Team Manager, User | 3 base roles (OWNER, ADMIN, MEMBER) + PBAC custom roles | **Low** | Cal.com's PBAC enables unlimited custom roles with fine-grained permissions |
...
| **Managed event types** | Admin-created, lockable, assignable to users/groups | `SchedulingType.MANAGED` — admin-pushed templates with team cascading | **Low** | Both platforms support managed events; functionally equivalent |
...
| **Team event routing** | Round Robin (even/priority), Collective events | Round Robin, Collective, plus Managed and Dynamic scheduling types | **Low** | Cal.com exceeds — more scheduling paradigms |
| **Member invitation** | Email-based, auto-resend 7 days, up to 100 at once | Token-based invitations with onboarding URL, batch email operations, seat accounting | **Low** | Functionally equivalent |
...
|--------|-------------|----------|----------------|-------------------|----------------|
| **AT-001** | Cal.com lacks a named "Group Admin" role equivalent in the UI | **Low** | `packages/features/pbac/` | Calendly's 5-role model with Group Admin | PBAC already supports creating custom roles with group-level permissions. Consider adding a default "Group Admin" role template for easier adoption. |
| **AT-002** | Cal.com lacks a named "Team Manager" role in the UI | **Low** | `packages/features/pbac/` | Calendly's Team Manager role | PBAC supports this via custom roles. Consider adding a default "Team Manager" role template. |
| **AT-003** | User CSV export not a built-in admin feature | **Low** | `packages/features/ee/organizations/` | Calendly's Admin Center CSV export | Consider adding a user export endpoint or admin UI feature. Low priority — data is accessible via API. |
| **AT-004** | SCIM group provisioning documentation less mature | **Low** | `packages/features/ee/organizations/` | Calendly's SCIM group provisioning with IdP attribute mapping | Enhance SCIM documentation and potentially add group-aware SCIM endpoints. Cal.com's domain-based auto-join partially compensates. |
| **AT-ADV-001** | **Cal.com advantage:** Hierarchical organizations with sub-teams | **N/A** | `packages/features/ee/organizations/`, `packages/features/ee/teams/` | Not available in Calendly | Cal.com's multi-level org hierarchy is a significant enterprise advantage. |
| **AT-ADV-002** | **Cal.com advantage:** PBAC with custom roles and fine-grained permissions | **N/A** | `packages/features/pbac/` | Not available in Calendly (fixed 5-role model) | PBAC with resource.action permissions, wildcard support, and custom roles far exceeds Calendly's fixed role model. |
| **AT-ADV-003** | **Cal.com advantage:** Programmatic organization management via API | **N/A** | `packages/features/ee/organizations/`, `apps/api/v2/` | Calendly API is read-only for org management | Cal.com's API v2 supports full CRUD for organizations, teams, and members. |
| **AT-ADV-004** | **Cal.com advantage:** Multi-organization membership | **N/A** | `packages/features/membership/` | Calendly restricts users to one organization | Cal.com allows users to belong to multiple organizations simultaneously. |
| **AT-ADV-005** | **Cal.com advantage:** Per-organization branding (logo, colors, banner) | **N/A** | `packages/features/ee/organizations/repositories/OrganizationRepository.ts` | Calendly supports org-wide logo only | Cal.com provides richer branding with bio, brandColor, bannerUrl, and sub-domain customization. |
| **AT-ADV-006** | **Cal.com advantage:** Dependency injection architecture | **N/A** | `packages/features/ee/organizations/di/` | Calendly is closed-source | Cal.com's DI architecture enables testable, modular enterprise governance code. |
...
<Steps>
  <Step title="Add Default Role Templates (Low Priority)">
    Create pre-configured PBAC role templates for "Group Admin" and "Team Manager" that map to Calendly's named roles. This improves adoption for teams migrating from Calendly without requiring PBAC expertise.
    
    `Target: packages/features/pbac/lib/constants.ts`
  </Step>
  <Step title="Enhance SCIM Group Provisioning Documentation (Low Priority)">
    Document Cal.com's SCIM integration patterns more prominently, particularly group-aware provisioning for enterprise customers migrating from Calendly's SCIM setup.
    
    `Target: packages/features/ee/organizations/README.md`
  </Step>
  <Step title="Add Admin User Export Feature (Low Priority)">
    Implement a CSV/JSON export endpoint for organization member lists in the API v2, matching Calendly's Admin Center export convenience.
    
    `Target: apps/api/v2/src/modules/organizations/`
  </Step>
  <Step title="Document PBAC Migration Path for Calendly Users">
    Create a migration guide mapping Calendly's five fixed roles to recommended Cal.com PBAC configurations, helping enterprise customers transition their governance model.
...
The following behavioral acceptance criteria define what constitutes admin governance parity with Calendly. Given Cal.com's existing capabilities, most criteria are **already met or exceeded**.
...
- [ ] Default "Group Admin" and "Team Manager" role templates available (enhancement — AT-001, AT-002)

### Team Management

- [x] Teams can be created within organizations
- [x] Sub-teams can be created within teams
- [x] Team event types support Round Robin, Collective, and Managed scheduling
...
- [x] Managed events are pushed to team members
...
- [ ] SCIM group provisioning with IdP attribute mapping (enhancement — AT-004)
...
- [ ] Admin CSV/JSON export of organization member lists (enhancement — AT-003)
