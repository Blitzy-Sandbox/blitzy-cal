...
<Note>
  Gap IDs in this document use the `AVL-GAP-*` and `AVL-ADV-*` prefixes for domain-specific tracking. These map to the **`AV-*`** epic prefix used in the [Sprint Roadmap — Epic Catalog](/sprint-roadmap/epic-catalog). For example, `AVL-GAP-001` corresponds to implementation work tracked under the `AV` domain epics. See the [Epic ID Convention](/sprint-roadmap/epic-catalog#epic-id-convention) for the full prefix mapping.
</Note>
...
- [Gap Report — Event Types](/gap-report/event-types) — Event type configuration gap analysis with booking window overlap
- [Gap Report — Calendar Integrations](/gap-report/calendar-integrations) — Calendar integration gap analysis with busy time aggregation overlap
- [Sprint Roadmap — Epic Catalog](/sprint-roadmap/epic-catalog) — All `AV-*` epics for availability domain gap closure with priority and dependencies
- [Sprint Roadmap — Validation Criteria](/sprint-roadmap/validation-criteria) — Parity validation checklists for availability acceptance criteria
