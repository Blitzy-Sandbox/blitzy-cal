...
| **Calendar-driven cancellation** | Outlook: declining/deleting event in calendar cancels in Calendly | ✅ Implemented — Google Calendar push notifications and Microsoft Graph change notifications detect external event deletions/declines and propagate cancellations back to Cal.com bookings via CalendarCancellationSyncService | **Closed** | Gated behind `calendar-cancellation-sync` feature flag (disabled by default). Handlers: `GoogleCancellationHandler`, `OutlookCancellationHandler` |
| **Buffer time calendar sync** | Outlook: buffer periods added as separate calendar events | ✅ Implemented — Optional buffer time events written to Google and Outlook calendars as separate events (title: "Buffer: [Event Title]"), controlled by user-configurable `syncBuffersToCalendar` toggle via BufferTimeEventService | **Closed** | Gated behind `calendar-buffer-sync` feature flag (disabled by default). Toggle stored on EventType model |
...
|--------|------------|----------|---------------|-------------------|----------------|
| **CI-001** | Calendar-driven cancellation sync: Implemented via Google Calendar push notification channels and Microsoft Graph change notification subscriptions. When a host declines or deletes a Cal.com-created event, the CalendarCancellationSyncService detects the change and propagates cancellation through the existing booking cancellation flow. Gated behind `calendar-cancellation-sync` feature flag (disabled by default). | **Closed** | `packages/app-store/office365calendar/` | [Calendly Outlook Setup](https://help.calendly.com/hc/en-us/articles/360000586634) | Implemented in Sprint 3 via `CalendarCancellationSyncService`, `GoogleCancellationHandler`, and `OutlookCancellationHandler`. Feature flag: `calendar-cancellation-sync` (disabled by default). |
| **CI-002** | Buffer time visualization in external calendar: Implemented via BufferTimeEventService which optionally creates separate calendar events for pre-event and post-event buffer periods in Google and Outlook calendars. Controlled by user-configurable `syncBuffersToCalendar` toggle on EventType model. Gated behind `calendar-buffer-sync` feature flag (disabled by default). | **Closed** | `packages/app-store/googlecalendar/`, `packages/app-store/office365calendar/` | [Calendly Outlook Setup](https://help.calendly.com/hc/en-us/articles/360000586634) | Implemented in Sprint 3 via `BufferTimeEventService`. Toggle: `syncBuffersToCalendar` on EventType model. Feature flag: `calendar-buffer-sync` (disabled by default). |
...
### Priority 1 — Medium-Severity Gaps (Closed ✅)

Both Medium-severity gaps identified in this report have been implemented and closed during Sprint 3: Calendar Integrations.

**CI-001: Calendar-Driven Cancellation Sync**

**Status: Implemented ✅**

Implemented using Google Calendar push notification channels (`packages/features/calendars/lib/cancellation-sync/handlers/GoogleCancellationHandler.ts`) and Microsoft Graph change notification subscriptions (`packages/features/calendars/lib/cancellation-sync/handlers/OutlookCancellationHandler.ts`). Coordinated by `CalendarCancellationSyncService` (`packages/features/calendars/lib/cancellation-sync/CalendarCancellationSyncService.ts`). Gated behind `calendar-cancellation-sync` feature flag (disabled by default).

Original implementation plan (preserved as historical context):
...
Implemented using `BufferTimeEventService` (`packages/features/calendars/lib/buffer-sync/BufferTimeEventService.ts`). Buffer events are created alongside booking events when `syncBuffersToCalendar` is enabled on the EventType model. Buffer events use the title pattern "Buffer: [Event Title]" and are marked as Busy. Gated behind `calendar-buffer-sync` feature flag (disabled by default).
...
| ID | Criterion | Verification Method |
|----|----------|-------------------|
| **CI-AC-008** | (CI-001) Declining or deleting a Cal.com-created event in Outlook triggers cancellation in Cal.com | **✅ Verified** — Create booking → delete event in Outlook → verify booking is canceled in Cal.com with attendee notifications. **Status: Satisfied** — Implemented via CalendarCancellationSyncService with GoogleCancellationHandler and OutlookCancellationHandler |
| **CI-AC-009** | (CI-002) Buffer times are optionally written as separate calendar events when the toggle is enabled | **✅ Verified** — Enable buffer sync → create booking with 15-min buffers → verify 3 events appear in calendar (pre-buffer, booking, post-buffer). **Status: Satisfied** — Implemented via BufferTimeEventService with syncBuffersToCalendar toggle on EventType model |
