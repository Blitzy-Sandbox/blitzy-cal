...
| **Calendar-driven cancellation** | Outlook: declining/deleting event in calendar cancels in Calendly | ❌ Not currently implemented — cancellation initiated from Cal.com UI or API only | **Medium** | Calendly's Outlook integration supports bi-directional cancellation sync |
| **Buffer time calendar sync** | Outlook: buffer periods added as separate calendar events | ❌ Buffer times enforced during slot generation but not written as separate calendar events | **Medium** | Calendly writes visual buffer blocks to the external calendar |
...
|--------|------------|----------|---------------|-------------------|----------------|
| **CI-001** | Calendar-driven cancellation sync: Calendly's Outlook integration supports declining/deleting a calendar event to trigger cancellation in the platform | **Medium** | `packages/app-store/office365calendar/` | [Calendly Outlook Setup](https://help.calendly.com/hc/en-us/articles/360000586634) | Implement webhook listener for Microsoft Graph change notifications to detect event deletions/declines and propagate cancellation to Cal.com bookings |
| **CI-002** | Buffer time visualization in external calendar: Calendly writes buffer periods as separate calendar events for visual clarity | **Medium** | `packages/app-store/googlecalendar/`, `packages/app-store/office365calendar/` | [Calendly Outlook Setup](https://help.calendly.com/hc/en-us/articles/360000586634) | Optionally create buffer time events in external calendars when booking events are created, with a user-configurable toggle |
...
### Priority 1 — Close Medium-Severity Gaps

**CI-001: Calendar-Driven Cancellation Sync**

Implement support for detecting when a host declines or deletes a Calendly-created event in their external calendar and propagating that cancellation back to Cal.com. This requires:
...
Optionally write buffer time periods as separate calendar events to provide hosts with visual clarity about their unavailable time:
...
| ID | Criterion | Verification Method |
|----|----------|-------------------|
| **CI-AC-008** | (CI-001) Declining or deleting a Cal.com-created event in Outlook triggers cancellation in Cal.com | Create booking → delete event in Outlook → verify booking is canceled in Cal.com with attendee notifications |
| **CI-AC-009** | (CI-002) Buffer times are optionally written as separate calendar events when the toggle is enabled | Enable buffer sync → create booking with 15-min buffers → verify 3 events appear in calendar (pre-buffer, booking, post-buffer) |
