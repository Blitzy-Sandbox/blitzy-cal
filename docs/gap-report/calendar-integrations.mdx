...
<Info>
  **Key Finding:** Cal.com **significantly exceeds** Calendly in calendar integration breadth and flexibility. Cal.com provides **11+ calendar adapters** with bi-directional sync, per-event-type calendar selection, and AES-256 credential encryption. Calendly currently supports **3 calendar providers** (Google, Office 365/Outlook, Exchange) and dropped iCloud support for new connections in August 2024. Two previously identified Medium-severity gaps (calendar-driven cancellation sync and buffer time visualization) have been **implemented behind feature flags** during Sprint 3 — `CalendarCancellationSyncService` with `GoogleCancellationHandler` and `OutlookCancellationHandler` handle calendar-driven cancellation sync (gated behind `calendar-cancellation-sync`), and `BufferTimeEventService` with `CalendarEventBuilder.buildBufferEvent()` handle buffer time visualization (gated behind `calendar-buffer-sync`).
</Info>
...
| **Calendar-driven cancellation** | Outlook: declining/deleting event in calendar cancels in Calendly | ✅ Completed (feature-flagged) — `CalendarCancellationSyncService` orchestrates cancellation propagation from external calendar event deletions/declines back to Cal.com bookings. `GoogleCancellationHandler` processes Google Calendar push notification payloads. `OutlookCancellationHandler` processes Microsoft Graph change notification payloads. `handleCancelBooking` accepts `source: "external_calendar"` parameter, DI tokens defined, feature flag `calendar-cancellation-sync` inserted (disabled by default), schema column `externalCancellationSyncEnabled` added to Credential model. | **Medium** | Gated behind `calendar-cancellation-sync` feature flag (disabled by default). |
| **Buffer time calendar sync** | Outlook: buffer periods added as separate calendar events | ✅ Completed (feature-flagged) — `BufferTimeEventService` creates, updates, and deletes buffer time events in external calendars alongside booking events. `CalendarEventBuilder.buildBufferEvent()` constructs buffer event `CalendarEvent` objects. Schema column `syncBuffersToCalendar` added to EventType model, feature flag `calendar-buffer-sync` inserted (disabled by default), DI tokens defined. | **Medium** | Gated behind `calendar-buffer-sync` feature flag (disabled by default). |
...
|--------|------------|----------|---------------|-------------------|----------------|
| **CI-001** | Calendar-driven cancellation sync: Implemented behind `calendar-cancellation-sync` feature flag (disabled by default). `CalendarCancellationSyncService` (`packages/features/calendars/lib/cancellation-sync/CalendarCancellationSyncService.ts`) orchestrates cancellation propagation from external calendar event deletions/declines back to Cal.com bookings. `GoogleCancellationHandler` (`packages/features/calendars/lib/cancellation-sync/handlers/GoogleCancellationHandler.ts`) processes Google Calendar push notifications. `OutlookCancellationHandler` (`packages/features/calendars/lib/cancellation-sync/handlers/OutlookCancellationHandler.ts`) processes Microsoft Graph change notifications. `handleCancelBooking` accepts `source: "external_calendar"` parameter. Schema column `externalCancellationSyncEnabled` added to Credential model. | **Medium** | `packages/app-store/googlecalendar/`, `packages/app-store/office365calendar/` | [Calendly Outlook Setup](https://help.calendly.com/hc/en-us/articles/360000586634) | Sprint 3 completed. Feature flag: `calendar-cancellation-sync` (disabled by default). |
| **CI-002** | Buffer time visualization in external calendar: Implemented behind `calendar-buffer-sync` feature flag (disabled by default). `BufferTimeEventService` (`packages/features/calendars/lib/buffer-sync/BufferTimeEventService.ts`) creates, updates, and deletes buffer time events in external calendars. `CalendarEventBuilder.buildBufferEvent()` constructs buffer event `CalendarEvent` objects with "Buffer:" title prefix. Schema column `syncBuffersToCalendar` added to EventType model. | **Medium** | `packages/app-store/googlecalendar/`, `packages/app-store/office365calendar/` | [Calendly Outlook Setup](https://help.calendly.com/hc/en-us/articles/360000586634) | Sprint 3 completed. Feature flag: `calendar-buffer-sync` (disabled by default). |
...
### Priority 1 — Medium-Severity Gaps (Completed ✅)

Both Medium-severity gaps identified in this report have been fully implemented behind feature flags during Sprint 3.

**CI-001: Calendar-Driven Cancellation Sync**

**Status: Completed ✅ — Implemented behind `calendar-cancellation-sync` feature flag (disabled by default)**

Implementation completed:
- `CalendarCancellationSyncService` (`packages/features/calendars/lib/cancellation-sync/CalendarCancellationSyncService.ts`) — Core service handling the flow: external calendar event deletion detected → lookup corresponding Cal.com booking by external event UID → invoke `handleCancelBooking` with `source: "external_calendar"` indicator → dispatch attendee notifications and webhook events
- `GoogleCancellationHandler` (`packages/features/calendars/lib/cancellation-sync/handlers/GoogleCancellationHandler.ts`) — Processes Google Calendar push notification payloads, verifying event deletion or decline, and delegating to `CalendarCancellationSyncService`
- `OutlookCancellationHandler` (`packages/features/calendars/lib/cancellation-sync/handlers/OutlookCancellationHandler.ts`) — Processes Microsoft Graph change notification payloads for event deletion and decline detection

Integration infrastructure: `handleCancelBooking` accepts `source: "external_calendar"` parameter, DI tokens defined, feature flag `calendar-cancellation-sync` inserted (disabled by default), `externalCancellationSyncEnabled` nullable Boolean column added to the Credential model.

`Source: packages/features/calendars/lib/cancellation-sync/CalendarCancellationSyncService.ts`
`Source: packages/features/calendars/lib/cancellation-sync/handlers/GoogleCancellationHandler.ts`
`Source: packages/features/calendars/lib/cancellation-sync/handlers/OutlookCancellationHandler.ts`

**CI-002: Buffer Time Calendar Visualization**

**Status: Completed ✅ — Implemented behind `calendar-buffer-sync` feature flag (disabled by default)**

Implementation completed:
- `BufferTimeEventService` (`packages/features/calendars/lib/buffer-sync/BufferTimeEventService.ts`) — Creates, updates, and deletes buffer time events in external calendars alongside booking events. When the `syncBuffersToCalendar` toggle is enabled, creates additional calendar events for pre-event and post-event buffer periods. On cancellation, deletes buffer events alongside the main event. Buffer events are marked as "Busy" with a distinctive "Buffer:" title prefix.
- `CalendarEventBuilder.buildBufferEvent()` method — Constructs `CalendarEvent` objects for buffer time periods with appropriate metadata (title prefix, busy/free status, relationship to parent booking)

Integration infrastructure: `syncBuffersToCalendar` nullable Boolean column added to the EventType model, feature flag `calendar-buffer-sync` inserted (disabled by default), DI tokens defined.

`Source: packages/features/calendars/lib/buffer-sync/BufferTimeEventService.ts`
`Source: packages/features/CalendarEventBuilder.ts`
...
| ID | Criterion | Verification Method |
|----|----------|-------------------|
| **CI-AC-008** | (CI-001) Declining or deleting a Cal.com-created event in Outlook triggers cancellation in Cal.com | ✅ Implemented (feature-flagged) — `CalendarCancellationSyncService`, `GoogleCancellationHandler`, and `OutlookCancellationHandler` handle cancellation propagation from external calendar event deletions/declines. `handleCancelBooking` accepts `source: "external_calendar"` parameter. Gated behind `calendar-cancellation-sync` feature flag (disabled by default). Verification: Create booking → delete event in Outlook → verify booking is canceled in Cal.com with attendee notifications. |
| **CI-AC-009** | (CI-002) Buffer times are optionally written as separate calendar events when the toggle is enabled | ✅ Implemented (feature-flagged) — `BufferTimeEventService` creates buffer time events alongside booking events. `CalendarEventBuilder.buildBufferEvent()` constructs buffer event objects with "Buffer:" title prefix. Gated behind `calendar-buffer-sync` feature flag (disabled by default). Verification: Enable buffer sync → create booking with 15-min buffers → verify 3 events appear in calendar (pre-buffer, booking, post-buffer). |
