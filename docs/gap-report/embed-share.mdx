...
- Validation criteria satisfied: **EM-VAL-001** (inline render), **EM-VAL-007** (prefill parameters)
...
- Validation criteria satisfied: **EM-VAL-002** (modal overlay), **EM-VAL-008** (prerendering)
- Implementation reference: `packages/embeds/embed-core/src/embed.ts`
...
- Validation criteria satisfied: **EM-VAL-004** (React wrapper), **EM-VAL-005** (vanilla JS snippet), **EM-VAL-006** (postMessage protocol)
...
<Info>
  **Validation Status:** The acceptance criteria below map to validation criteria **EM-VAL-001** through **EM-VAL-009** as defined in the [Sprint Roadmap — Validation Criteria](/sprint-roadmap/validation-criteria). Validation is pending Sprint 6 implementation completion.
</Info>

### Core Embed Functionality

- [ ] **AC-EMB-01**: Inline embed renders a fully functional booking widget within a host page container element *(EM-001, EM-VAL-001)*
- [ ] **AC-EMB-02**: Modal embed opens a booking overlay when triggered by button click or programmatic API call *(EM-002, EM-VAL-002)*
- [ ] **AC-EMB-03**: Floating button embed displays a persistent button that opens the modal on click *(EM-003, EM-VAL-003)*
- [ ] **AC-EMB-04**: All three embed types support prefilling name, email, and custom fields *(EM-001, EM-002, EM-003, EM-VAL-007)*
- [ ] **AC-EMB-05**: Embed auto-resizes to match content height without host page scrollbars *(EM-001, EM-VAL-001)*

### Performance and Reliability

- [ ] **AC-EMB-06**: Prerendered modal displays within 100ms of CTA click (no network wait) *(EM-002, EM-VAL-008)*
- [ ] **AC-EMB-07**: Skeleton loader displays within 50ms of embed initialization for supported page types *(EM-001, EM-VAL-009)*
- [ ] **AC-EMB-08**: Command queue ensures zero commands are lost when API calls precede iframe readiness *(EM-001, EM-002, EM-003, EM-VAL-006)*
- [ ] **AC-EMB-09**: postMessage handshake completes reliably across Chrome, Firefox, Safari, and Edge *(EM-001, EM-002, EM-003, EM-VAL-006)*

### Multi-Embed and Isolation

- [ ] **AC-EMB-10**: Multiple embeds on the same page operate independently via namespace isolation *(EM-004, EM-VAL-004, EM-VAL-005)*
- [ ] **AC-EMB-11**: Each namespace maintains separate command queues, event listeners, and iframe references *(EM-004, EM-VAL-004, EM-VAL-005)*

### Tracking and Observability

- [ ] **AC-EMB-12**: `bookerViewed` event fires on first embed display (not during prerendering) *(EM-001, EM-002, EM-VAL-006)*
- [ ] **AC-EMB-13**: `bookerReady` event fires only when slots are fully loaded and selectable *(EM-001, EM-002, EM-VAL-006)*
- [ ] **AC-EMB-14**: `linkFailed` event fires with error code on page load failure *(EM-001, EM-002, EM-003, EM-VAL-006)*
- [ ] **AC-EMB-15**: Tracking events are suppressed during prerendering phase *(EM-002, EM-VAL-008)*

### Platform Compatibility

- [ ] **AC-EMB-16**: Embed functions correctly when loaded via embed-snippet on static HTML pages *(EM-004, EM-VAL-005)*
- [ ] **AC-EMB-17**: React component (`@calcom/embed-react`) integrates correctly in Next.js, Remix, and Vite React applications *(EM-004, EM-VAL-004)*
- [ ] **AC-EMB-18**: Embed functions with Content Security Policy (CSP) headers allowing the Cal.com origin *(EM-001, EM-002, EM-003, EM-VAL-006)*
