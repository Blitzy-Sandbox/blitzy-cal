...
| **Reconfirmation requests** | Dedicated reconfirmation Workflow template (paid plans) | ✅ Pre-built "Reconfirmation" workflow template added to workflow template library | **Low** | Gap closed (Sprint 8 — NF-001). Template sends email or SMS with confirmation link before event starts |
| **No-show workflow handling** | Mark as no-show halts follow-ups; triggers no-show-specific workflows | ✅ `INVITEE_NO_SHOW` workflow trigger event added; no-show detection integrated with workflow engine | **Low** | Gap closed (Sprint 8 — NF-003). Follow-up workflow halting implemented when no-show status set |
| **Email sending from user's address** | Can send Workflow emails from connected Gmail/Outlook accounts | ✅ Workflow email steps support "Send from my email" option using connected Gmail/Outlook OAuth tokens | **Low** | Gap closed (Sprint 8 — NF-001). Delegated email sending via calendar provider APIs |
...
|--------|-------------|----------|--------|---------------|-------------------|----------------|
| NF-001 | **Reconfirmation workflow template** — Calendly provides a dedicated reconfirmation Workflow template that requests invitees to confirm attendance; Cal.com can achieve this via custom workflows but lacks a first-class reconfirmation template | Medium | ✅ Closed | `packages/features/ee/workflows/` | [Workflows Overview](https://help.calendly.com/hc/en-us/articles/360051017814-Workflows-overview) | Pre-built "Reconfirmation" workflow template added to workflow template library. Template sends email or SMS with confirmation link before event starts. Implementation: `packages/features/ee/workflows/lib/` |
| NF-002 | **No-show workflow integration** — Calendly allows marking invitees as no-shows, which halts follow-up workflows and triggers no-show-specific workflows; Cal.com detects no-shows (via webhook events) but lacks integrated no-show workflow management | Medium | ✅ Closed | `packages/features/ee/workflows/` | [Workflows Overview](https://help.calendly.com/hc/en-us/articles/360051017814-Workflows-overview) | `INVITEE_NO_SHOW` added as workflow trigger event. No-show detection integrated with workflow engine. Follow-up workflow halting implemented. Implementation: `packages/features/ee/workflows/lib/` |
| NF-003 | **User-address email sending** — Calendly Workflows can send emails from the user's connected Gmail or Outlook account; Cal.com sends from the configured SMTP/provider address | Medium | ✅ Closed | `packages/emails/` | [Calendly Workflows Blog](https://calendly.com/blog/3-new-workflows-improvements) | Workflow email steps now support "Send from my email" option using connected Gmail/Outlook OAuth tokens. Implementation: `packages/emails/email-manager.ts` |
...
- Pre-built "Reconfirmation" workflow template added to the workflow template library (closes gap inventory NF-001)
...
- `INVITEE_NO_SHOW` added as a workflow trigger event (closes gap inventory NF-002)
...
### Priority 1: Reconfirmation Workflow Template (NF-001) — ✅ Implemented

~~Add a pre-built "Reconfirmation" workflow template to Cal.com's workflow template library.~~ This template has been implemented:
...
Pre-built "Reconfirmation" workflow template added to the workflow template library, aligning Cal.com with Calendly's dedicated reconfirmation workflow feature. Leverages the existing workflow engine infrastructure in `packages/features/ee/workflows/`.

`Source: packages/features/ee/workflows/lib/`

### Priority 2: No-Show Workflow Integration (NF-002) — ✅ Implemented

~~Integrate no-show detection with the workflow engine.~~ No-show workflow integration has been completed:

- ✅ `INVITEE_NO_SHOW` added as a `WorkflowTriggerEvents` enum value
- ✅ No-show workflow template fires when an invitee is marked as no-show
- ✅ Pending follow-up workflows are halted when no-show status is set
- ✅ "Reschedule prompt" action included in the no-show workflow template

Cal.com's existing no-show detection via the `NO_SHOW_HOST` webhook trigger event has been extended to the workflow engine, providing a streamlined no-show management experience matching Calendly's behavior.

`Source: packages/features/ee/workflows/lib/`

### Priority 3: User-Address Email Sending (NF-003) — ✅ Implemented

~~Evaluate allowing workflow emails to be sent from the user's connected calendar email address.~~ User-address email sending has been implemented:

- ✅ Leverages existing OAuth tokens from connected calendars in `packages/app-store/googlecalendar/` and `packages/app-store/office365calendar/`
- ✅ "Send from my email" option added in workflow step configuration
- ✅ Delegated email sending implemented through the calendar provider's API (Gmail API `users.messages.send` or Microsoft Graph `sendMail`)

OAuth scope handling and token refresh are managed transparently. Delivery failures fall back to the default SMTP/provider address.
