...
The following sprint epics (NF-001 through NF-004) were implemented to close identified gaps and achieve Calendly notification parity. Validation criteria NF-VAL-001 through NF-VAL-010 apply across these epics.
...
- **Validation criteria:** NF-VAL-001, NF-VAL-002, NF-VAL-003
...
- **Validation criteria:** NF-VAL-004, NF-VAL-005
...
- **Validation criteria:** NF-VAL-006, NF-VAL-007, NF-VAL-008
...
- **Validation criteria:** NF-VAL-009, NF-VAL-010
...
The following behavioral acceptance criteria define when Cal.com achieves full notification parity with Calendly. Validation criteria IDs NF-VAL-001 through NF-VAL-010 from `docs/sprint-roadmap/validation-criteria.mdx` apply across all sprint epics below.

### Reconfirmation Workflow (NF-001) — ✅ All Criteria Met
- [x] A pre-built "Reconfirmation" workflow template exists in the workflow template library *(NF-VAL-001)*
- [x] The template sends an email or SMS with a confirmation link before the event starts *(NF-VAL-002)*
- [x] Confirmation status is visible on the bookings management page *(NF-VAL-003)*
- [x] The template supports configurable time offsets (e.g., 24 hours, 1 hour before event) *(NF-VAL-002)*

### No-Show Workflow Integration (NF-002) — ✅ All Criteria Met
- [x] Marking an invitee as no-show triggers a `NO_SHOW` workflow event *(NF-VAL-006)*
- [x] Pending follow-up workflows are halted when no-show status is set *(NF-VAL-007)*
- [x] A pre-built "No-Show Reschedule" workflow template sends a rescheduling prompt to the no-show invitee *(NF-VAL-008)*
- [x] No-show invitees do not receive standard post-meeting workflows (thank-you, follow-up) *(NF-VAL-007)*

### User-Address Email Sending (NF-003) — ✅ All Criteria Met
- [x] Workflow email steps include a "Send from my email" option *(NF-VAL-004)*
- [x] Emails sent via this option use the user's connected Gmail or Outlook account *(NF-VAL-004)*
- [x] OAuth token refresh is handled transparently *(NF-VAL-005)*
- [x] Delivery failures fall back to the default SMTP/provider address *(NF-VAL-005)*
