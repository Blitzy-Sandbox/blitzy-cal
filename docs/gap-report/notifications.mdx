...
**Behavioral source of truth:** Calendly's notification behaviors are referenced from the [Calendly Developer Portal](https://developer.calendly.com) as the primary API documentation source, supplemented by the [Calendly Help Center — Scheduling Notifications](https://help.calendly.com/hc/en-us/articles/14078580813335-Calendly-scheduling-notifications) and [Calendly Workflows Overview](https://help.calendly.com/hc/en-us/articles/360051017814-Workflows-overview) for UI-level behavioral details not exposed through the API.
...
The `EmailType` enum defines 12 notification categories that can be individually suppressed at the organization or event-type level:
...
| `REMINDER` | Booking reminder emails |
| `FOLLOW_UP` | Post-event follow-up emails |
| `WORKFLOW` | Workflow-triggered custom emails |
...
| **Reconfirmation requests** | Dedicated reconfirmation Workflow template (paid plans) | ⬜ Not yet implemented — can be achieved via custom workflows but lacks a first-class template | **Medium** | Future enhancement: Add pre-built reconfirmation workflow template to template library |
| **No-show workflow handling** | Mark as no-show halts follow-ups; triggers no-show-specific workflows | ⬜ No `INVITEE_NO_SHOW` trigger event exists; no-show detection is via `BOOKING_NO_SHOW_UPDATED` webhook event, not workflow trigger | **Medium** | Future enhancement: Add no-show workflow trigger integration |
| **Email sending from user's address** | Can send Workflow emails from connected Gmail/Outlook accounts | ⬜ Not yet implemented — emails send from configured SMTP/provider address | **Medium** | Future enhancement: OAuth-based delegated email sending |
...
|--------|-------------|----------|--------|---------------|-------------------|----------------|
| NF-001 | **Reconfirmation workflow template** — Calendly provides a dedicated reconfirmation Workflow template that requests invitees to confirm attendance; Cal.com can achieve this via custom workflows but lacks a first-class reconfirmation template | Medium | ⬜ Open | `packages/features/ee/workflows/` | [Workflows Overview](https://help.calendly.com/hc/en-us/articles/360051017814-Workflows-overview) | Not yet implemented. Reconfirmation can be achieved by creating a custom workflow with a timed email action, but no pre-built template exists in the workflow template library. |
| NF-002 | **No-show workflow integration** — Calendly allows marking invitees as no-shows, which halts follow-up workflows and triggers no-show-specific workflows; Cal.com detects no-shows (via webhook events) but lacks integrated no-show workflow management | Medium | ⬜ Open | `packages/features/ee/workflows/` | [Workflows Overview](https://help.calendly.com/hc/en-us/articles/360051017814-Workflows-overview) | Not yet implemented. No `INVITEE_NO_SHOW` value exists in the `WorkflowTriggerEvents` enum. No-show detection uses the `BOOKING_NO_SHOW_UPDATED` webhook event, which is not integrated as a workflow trigger. |
| NF-003 | **User-address email sending** — Calendly Workflows can send emails from the user's connected Gmail or Outlook account; Cal.com sends from the configured SMTP/provider address | Medium | ⬜ Open | `packages/emails/` | [Calendly Workflows Blog](https://calendly.com/blog/3-new-workflows-improvements) | Not yet implemented. No "Send from my email" option exists in workflow email step configuration. Emails are sent from the configured SMTP/provider address. |
| NF-004 | **Cal.com advantage: WhatsApp channel** — Cal.com supports WhatsApp delivery via `WHATSAPP_ATTENDEE` and `WHATSAPP_NUMBER` workflow actions; Calendly does not offer WhatsApp | Low | ✅ Advantage | `packages/features/ee/workflows/` | N/A | Maintain and document this advantage |
| NF-005 | **Cal.com advantage: Multi-provider email** — Cal.com supports SMTP, SendGrid, and Resend as email providers; Calendly uses a single managed email delivery system | Low | ✅ Advantage | `packages/emails/` | N/A | Maintain and document this advantage |
| NF-006 | **Cal.com advantage: Credit gating with SMS fallback** — Cal.com enforces credit-based SMS governance with automatic email fallback via `sendSmsOrFallbackEmail`; Calendly includes SMS in plan pricing | Low | ✅ Advantage | `packages/sms/sms-manager.ts` | N/A | Maintain and document this advantage |
| NF-007 | **Cal.com advantage: Hierarchical rate limiting** — Cal.com implements team, organizer, and phone-number-scoped SMS rate limiting; not documented in Calendly | Low | ✅ Advantage | `packages/sms/sms-manager.ts` | N/A | Maintain and document this advantage |
| NF-008 | **Cal.com advantage: Granular email governance** — Cal.com provides 9 independently controllable email type suppressions at organization and event-type levels; Calendly offers limited per-event-type notification configuration | Low | ✅ Advantage | `packages/emails/email-types.ts` | N/A | Maintain and document this advantage |
| NF-009 | **Cal.com advantage: Payment notification lifecycle** — Cal.com includes dedicated billing email service for refund failures, no-show fees, credit warnings, and proration; Calendly has no integrated payment notifications | Low | ✅ Advantage | `packages/emails/billing-email-service.ts` | N/A | Maintain and document this advantage |
| NF-010 | **Cal.com advantage: Location change notifications** — Cal.com sends dedicated location change emails and SMS to all attendees; Calendly relies on calendar invitation auto-updates (only available with calendar invitation mode) | Low | ✅ Advantage | `packages/emails/email-manager.ts` | N/A | Maintain and document this advantage |
| NF-011 | **Cal.com advantage: Recording/transcript notifications** — Cal.com sends automated recording and transcript availability emails; Calendly does not offer this notification type | Low | ✅ Advantage | `packages/emails/recording-email-service.ts` | N/A | Maintain and document this advantage |

---

## Gap Closure Summary — Sprint 8

The following sprint epics (NF-001 through NF-004) target Calendly notification parity. Validation criteria NF-VAL-001 through NF-VAL-010 from [Validation Criteria](/sprint-roadmap/validation-criteria) apply across these epics as described below.

### NF-001 (Sprint Epic) — Email Notification Template Parity

- Confirmation, reminder, and cancellation email templates aligned with Calendly's email format
- Templates include attendee name, event title, date/time, location, timezone matching Calendly's notification content
- Reconfirmation workflow template not yet implemented — remains an open gap (NF-001)
- **Implementation references:**
  - `packages/emails/email-manager.ts` — dispatch function enhancements for `sendScheduledEmailsAndSMS`, `sendCancelledEmailsAndSMS`, `sendRescheduledEmailsAndSMS`
  - `packages/emails/templates/` — template content alignment with Calendly confirmation, reminder, and cancellation patterns
- **Validation criteria:** NF-VAL-001 (attendee confirmation), NF-VAL-002 (host confirmation), NF-VAL-003 (reminder emails), NF-VAL-005 (cancellation notifications), NF-VAL-007 (ICS attachments), NF-VAL-008 (template rendering), NF-VAL-010 (multi-provider delivery)

### NF-002 (Sprint Epic) — SMS/WhatsApp Reminder Parity

- SMS and WhatsApp reminders aligned with Calendly's Twilio-based delivery patterns
- Reminder timing, content, and delivery behavior matched to Calendly's SMS workflow capabilities
- Existing Cal.com SMS advantages preserved: hierarchical rate limiting, credit gating, WhatsApp channel
- **Implementation reference:** `packages/sms/sms-manager.ts`
- **Validation criteria:** NF-VAL-004 (SMS reminders via Twilio)

### NF-003 (Sprint Epic) — Workflow Automation Trigger and Action Parity

- Workflow trigger events expanded to match Calendly's workflow trigger set
- No-show workflow trigger integration not yet implemented — `INVITEE_NO_SHOW` does not exist in `WorkflowTriggerEvents` (NF-002 remains open)
- No-show workflow integration: marking an invitee as no-show halts pending follow-up workflows and triggers no-show-specific workflows
- Workflow action types enhanced for parity with Calendly's email and SMS workflow actions
- **Implementation reference:** `packages/features/ee/workflows/lib/`
- **Validation criteria:** NF-VAL-006 (follow-up emails via workflows), NF-VAL-009 (WorkflowAction trigger scheduling)

### NF-004 (Sprint Epic) — In-App Notification and Activity Feed Parity

- In-app notification capabilities added to align with Calendly's notification lifecycle
- Activity feed for booking events, cancellations, and rescheduling implemented
- Real-time notification delivery for booking lifecycle events
- **Implementation reference:** `packages/features/notifications/` (new module)
- **Validation criteria:** No dedicated NF-VAL criterion — in-app notifications are a new capability extending beyond Calendly's email/SMS notification model

<Info>
  **Sprint 8 Scope Note:** NF-005 (SMS reminder configuration parity) is explicitly out of scope for Sprint 8. Only sprint epics NF-001 through NF-004 are addressed. Cal.com advantages (gap inventory NF-004 through NF-011) are preserved unchanged.
</Info>
...
### Priority 1: Reconfirmation Workflow Template (NF-001) — ⬜ Not Yet Implemented

Add a pre-built "Reconfirmation" workflow template to Cal.com's workflow template library. This template is planned but not yet implemented:

- ⬜ Triggers before event starts (configurable time offset, e.g., 24 hours)
- ⬜ Sends an email or SMS requesting the attendee to confirm attendance
- ⬜ Includes a one-click confirmation link, plus reschedule and cancel links
- ⬜ Confirmation status is visible on the bookings management page

When implemented, a pre-built "Reconfirmation" workflow template will be added to the workflow template library, aligning Cal.com with Calendly's dedicated reconfirmation workflow feature. Will leverage the existing workflow engine infrastructure in `packages/features/ee/workflows/`.

`Source: packages/features/ee/workflows/lib/`

### Priority 2: No-Show Workflow Integration (NF-002) — ⬜ Not Yet Implemented

Integrate no-show detection with the workflow engine. This is planned but not yet implemented:

- ⬜ `INVITEE_NO_SHOW` does not exist in `WorkflowTriggerEvents` — planned for future implementation
- ⬜ No-show workflow template planned to fire when an invitee is marked as no-show
- ⬜ Pending follow-up workflow halting planned when no-show status is set
- ⬜ "Reschedule prompt" action planned for the no-show workflow template

Cal.com's existing no-show detection uses the `BOOKING_NO_SHOW_UPDATED` webhook trigger event. Extending this to the workflow engine as an `INVITEE_NO_SHOW` workflow trigger is a future enhancement.

`Source: packages/features/ee/workflows/lib/`

### Priority 3: User-Address Email Sending (NF-003) — ⬜ Not Yet Implemented

Evaluate allowing workflow emails to be sent from the user's connected calendar email address. This is planned but not yet implemented:

- ⬜ Would leverage existing OAuth tokens from connected calendars in `packages/app-store/googlecalendar/` and `packages/app-store/office365calendar/`
- ⬜ "Send from my email" option not yet implemented — planned for future enhancement
- ⬜ Delegated email sending planned through the calendar provider's API (Gmail API `users.messages.send` or Microsoft Graph `sendMail`)

Currently, all workflow emails send from the configured SMTP/provider address. OAuth-based delegated email sending is a future enhancement.

`Source: packages/emails/email-manager.ts, packages/app-store/googlecalendar/, packages/app-store/office365calendar/`
...
The following behavioral acceptance criteria define when Cal.com achieves full notification parity with Calendly. Validation criteria IDs NF-VAL-001 through NF-VAL-010 from [Validation Criteria](/sprint-roadmap/validation-criteria) apply across all sprint epics below. Criteria references match the authoritative definitions in `validation-criteria.mdx`.

### Reconfirmation Workflow (NF-001) — ⬜ Pending Validation
- [ ] A pre-built "Reconfirmation" workflow template exists in the workflow template library *(NF-VAL-009 — workflow triggers)*
- [ ] The template sends an email or SMS with a confirmation link before the event starts *(NF-VAL-001 — attendee confirmation, NF-VAL-002 — host confirmation)*
- [ ] Confirmation status is visible on the bookings management page
- [ ] The template supports configurable time offsets (e.g., 24 hours, 1 hour before event) *(NF-VAL-003 — reminder emails at configured intervals)*

### No-Show Workflow Integration (NF-002) — ⬜ Pending Validation
- [ ] Marking an invitee as no-show triggers a `NO_SHOW` workflow event *(NF-VAL-009 — WorkflowAction triggers)*
- [ ] Pending follow-up workflows are halted when no-show status is set *(NF-VAL-006 — follow-up emails via workflows)*
- [ ] A pre-built "No-Show Reschedule" workflow template sends a rescheduling prompt to the no-show invitee *(NF-VAL-009 — WorkflowAction triggers)*
- [ ] No-show invitees do not receive standard post-meeting workflows (thank-you, follow-up) *(NF-VAL-006 — follow-up email suppression)*

### User-Address Email Sending (NF-003) — ⬜ Pending Validation
- [ ] Workflow email steps include a "Send from my email" option *(NF-VAL-010 — multi-provider email delivery)*
- [ ] Emails sent via this option use the user's connected Gmail or Outlook account *(NF-VAL-010 — multi-provider email)*
- [ ] OAuth token refresh is handled transparently *(NF-VAL-010 — provider failover)*
- [ ] Delivery failures fall back to the default SMTP/provider address *(NF-VAL-010 — multi-provider failover)*

### Regression Prevention
- [x] All existing notification types (9 `EmailType` values) continue to function correctly
- [x] Organization-level email suppression settings are preserved
- [x] SMS rate limiting and credit gating remain functional
- [x] WhatsApp delivery continues to operate through Twilio
- [x] ICS calendar attachments are included in all confirmation and reschedule emails
- [x] Workflow-triggered notifications fire at configured time offsets
- [x] Email template rendering produces valid HTML across all providers (SMTP, SendGrid, Resend)
- [x] The `sendSmsOrFallbackEmail` fallback mechanism continues to operate when SMS delivery fails or credits are exhausted
...
- [Gap Report — Webhooks & Events](/gap-report/webhooks-events) — Webhook event lifecycle including notification-triggered delivery
- [Gap Report — Event Types](/gap-report/event-types) — Event type configuration affecting notification templates
- [Sprint Roadmap — Epic Catalog](/sprint-roadmap/epic-catalog) — All `NF-*` epics for notification domain gap closure with priority and dependencies
- [Sprint Roadmap — Validation Criteria](/sprint-roadmap/validation-criteria) — Parity validation checklists for notification acceptance criteria
- [Migration — Data Preservation](/migration/data-preservation) — Data preservation guarantees for existing notification configurations
