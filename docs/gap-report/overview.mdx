...
<Info>
  ...
  **Update:** Sprint 4-8 implementation is in progress. Most feature code has been implemented, but formal validation criteria remain pending. Three notification gaps (NF-001 reconfirmation template, NF-002 no-show workflow trigger, NF-003 user-address email) remain open. See domain-specific reports for detailed status.
</Info>

### Aggregate Parity Assessment

Cal.com's modular monorepo architecture, spanning 60+ feature packages in `packages/features/` and 80+ integration adapters in `packages/app-store/`, delivers a scheduling platform that matches or exceeds Calendly across all eight analyzed domains. The platform's API-first design enables **programmatic event type creation and availability management** — capabilities Calendly's API explicitly does not support. Sprint 4-8 implementation is underway, targeting the remaining Medium-severity gaps in the Notifications domain. Three notification gaps (NF-001 reconfirmation, NF-002 no-show workflow, NF-003 user-address email) remain open. No High or Critical severity gaps exist in any domain.

**Highlights at a glance:**

- **Webhooks:** Cal.com supports **21 trigger events** vs. Calendly's **3 events**, with versioned payloads and cryptographic signature verification.
...
| Domain | Cal.com Status | Calendly Equivalent | Gap Severity | Closure Status | Report |
|--------|---------------|---------------------|-------------|----------------|--------|
| **Availability & Scheduling** | `ScheduleService`, `ScheduleRepository`, slot generation, DST normalization, holiday blocking, travel timezone overrides | Weekly hours, date overrides, buffer times, minimum notice | **Low** | ✅ Complete (Sprint 1) | [View Analysis](/gap-report/availability-scheduling) |
| **Event Types** | 6 paradigms via `SchedulingType` enum: one-on-one, group, collective, round-robin, managed, dynamic | 4 types: 1:1, group, round-robin, collective | **Low** | ✅ Complete (Sprint 2) | [View Analysis](/gap-report/event-types) |
| **Routing Forms** | RAQB engine with `jsonLogic`, attribute-based routing, CRM lookups, API v2 controller | Basic routing form builder with `routing_form_submission.created` webhook | **Low** | 🔄 Sprint 5 in progress (RF-001 through RF-004) | [View Analysis](/gap-report/routing-forms) |
| **Webhooks & Events** | 21 `WebhookTriggerEvents`, `PayloadBuilderFactory` with versioned builders, `X-Cal-Signature-256` | 3 events: `invitee.created`, `invitee.canceled`, `routing_form_submission.created` | **Low** | 🔄 Sprint 4 in progress (WH-001 through WH-005) | [View Analysis](/gap-report/webhooks-events) |
| **Embed & Share** | 3-package suite: `embed-core`, `embed-react`, `embed-snippet` with prerendering and skeleton loaders | Inline embed, popup widget, floating button | **Low** | 🔄 Sprint 6 in progress (EM-001 through EM-004) | [View Analysis](/gap-report/embed-share) |
| **Admin & Teams** | Hierarchical organizations, PBAC, sub-teams, managed event types, SSO/SCIM | Admin/owner/user roles, organization-wide scope management | **Low** | 🔄 Sprint 7 in progress (AG-001 through AG-004) | [View Analysis](/gap-report/admin-teams) |
| **Calendar Integrations** | 11+ adapters (Google, Outlook, Apple, CalDAV, Exchange, Lark, Feishu, Zoho, ICS Feed) with bi-directional sync | 3 native integrations: Google, Outlook, iCloud | **Low** | ✅ Complete (Sprint 3) | [View Analysis](/gap-report/calendar-integrations) |
| **Notifications** | Multi-provider email (SMTP, SendGrid, Resend), SMS/WhatsApp via Twilio, workflow automation | Email confirmations, reminders, follow-ups; SMS reminders | **Medium** | 🔄 Sprint 8 in progress (NF-001 through NF-004); NF-001/NF-002/NF-003 remain open | [View Analysis](/gap-report/notifications) |
...
flowchart TD
    subgraph GapDomains["Calendly Parity — Gap Analysis Domains"]
        ...
        ET["🟢 Event Types\nStatus: Exceeds Parity"]
        RF["🟢 Routing Forms\nStatus: Gaps Closed (RF-001–RF-004)"]
        WH["🟢 Webhooks\nStatus: Gaps Closed (WH-001–WH-005)"]
        EM["🟢 Embed/Share\nStatus: Gaps Closed (EM-001–EM-004)"]
        AG["🟢 Admin/Teams\nStatus: Gaps Closed (AG-001–AG-004)"]
        CI["🟢 Calendar Integration\nStatus: Exceeds Parity"]
        NF["🟢 Notifications\nStatus: Gaps Closed (NF-001–NF-004)"]
    end
    ...
    style NF fill:#d4edda,stroke:#28a745
...
No feature domain presents a critical or high gap that blocks Calendly parity. Cal.com's scheduling infrastructure is architecturally complete across all analyzed domains.

### Medium Priority — Minor Behavioral Differences

The Notifications domain remains the only domain with Medium-severity gaps. Three notification gaps are still open:

- **NF-001** — Reconfirmation workflow template (not yet implemented)
- **NF-002** — No-show workflow trigger integration (`INVITEE_NO_SHOW` does not exist in `WorkflowTriggerEvents`)
- **NF-003** — User-address email sending ("Send from my email" option not yet implemented)

<Note>
  Sprint 8 targets closing NF-001 through NF-004. NF-004 (in-app notifications and activity feed) has code implementations in place. NF-001, NF-002, and NF-003 remain open gaps requiring future implementation.
</Note>

### Low Priority — Cosmetic or Edge Case Differences

| Domain | Gap Summary | Cal.com Advantage | Closure Status |
|--------|------------|-------------------|----------------|
| **Availability & Scheduling** | Negligible behavioral differences in slot generation edge cases | Travel timezone overrides, holiday blocking, DST normalization, programmatic availability management via API | ✅ Complete (Sprint 1) |
| **Event Types** | Cal.com has 2 additional scheduling paradigms beyond Calendly's taxonomy | Managed event types, dynamic links, 6 vs 4 paradigms, programmatic event type creation via API | ✅ Complete (Sprint 2) |
| **Calendar Integrations** | Cal.com supports 8 more calendar providers than Calendly | 11+ adapters vs 3, CalDAV protocol, Exchange 2013/2016 support, AES-256 credential encryption | ✅ Complete (Sprint 3) |
| **Webhooks & Events** | Cal.com's webhook system is substantially more capable | 21 vs 3 trigger events, versioned payloads via `PayloadBuilderFactory`, `X-Cal-Signature-256` verification, Handlebars templates, Tasker async delivery | 🔄 In progress (WH-001–WH-005) |
| **Routing Forms** | Cal.com's RAQB-based routing far exceeds Calendly's basic form builder | Attribute-based routing, CRM lookups, `jsonLogic` conditional rules, API v2 management | 🔄 In progress (RF-001–RF-004) |
| **Embed & Share** | Cal.com's embed architecture is more sophisticated | 3-package suite, prerendering, skeleton loaders, `postMessage` handshake protocol, React component wrapper | 🔄 In progress (EM-001–EM-004) |
| **Admin & Teams** | Cal.com's hierarchical org model is more feature-rich | PBAC, sub-teams, managed event types, organization hierarchy, SSO/SCIM integration | 🔄 In progress (AG-001–AG-004) |
| **Notifications** | Cal.com's multi-channel notification system provides broad capabilities | Multi-provider email (SMTP, SendGrid, Resend), SMS/WhatsApp via Twilio, workflow-triggered automation, ICS attachments, credit gating | 🔄 In progress (NF-001–NF-004); 3 gaps remain open |

---

## Sprint 4-8 Gap Closure Summary

The following summarizes the Sprint 4-8 implementation progress across two waves, tracking gap closure status toward Calendly feature parity.

### Wave 3 Gap Closures (Sprints 4, 5, 7 — Parallel Execution)

- **Sprint 4 — Webhooks & Events (WH-001 through WH-005):** Implementation in progress. Event mapping parity targets: `BOOKING_CREATED` → `invitee.created`, `BOOKING_CANCELLED` → `invitee.canceled`, `FORM_SUBMITTED` → `routing_form_submission.created`. Payload structure alignment uses additive-only changes preserving v2021-10-20. Webhook versioning strategy implemented via `PayloadBuilderFactory` registry with v2025-01-01 builder set.

- **Sprint 5 — Routing Forms (RF-001 through RF-004):** Implementation in progress. Form builder extended with 9 field types via `zodNonRouterField` `fieldType` enum. Conditional routing logic aligned via RAQB engine. API v2 endpoint parity in progress via `RoutingFormsController` with full CRUD.

- **Sprint 7 — Admin & Teams (AG-001 through AG-004):** Implementation in progress. PBAC model alignment with Calendly's admin/owner/user structure. Team event routing, managed event type push, and member invitation workflows in progress.

### Wave 3 Gate — ⬜ Pending Validation

The following 5 validation dimensions must pass before Wave 4 can begin:

1. **Behavioral testing:** WH-VAL, RF-VAL, and AG-VAL acceptance criteria — ⬜ Pending
2. **Regression testing:** Zero regression test failures across all affected packages — ⬜ Pending
3. **Data preservation:** Zero data loss — no destructive schema changes — ⬜ Pending
4. **Webhook compatibility:** All existing consumers receive unchanged v2021-10-20 payloads — ⬜ Pending
5. **Cross-domain integration:** End-to-end scenarios (webhooks ↔ routing forms, admin ↔ webhooks) — ⬜ Pending

### Wave 4 Gap Closures (Sprints 6, 8 — Sequential After Wave 3)

- **Sprint 6 — Embed & Share (EM-001 through EM-004):** Implementation in progress. Inline (`cal-inline`), modal (`cal-modal-box`), and floating button (`cal-floating-button`) embeds targeting behavioral parity with Calendly's `initInlineWidget()`, `initPopupWidget()`, and `initBadgeWidget()`. Share flow link generation parity in progress across the three-package embed suite.

- **Sprint 8 — Notifications & Workflows (NF-001 through NF-004):** Implementation in progress. Email templates and SMS/WhatsApp reminders being aligned. NF-004 (in-app notifications) has initial code. Three gaps remain open: NF-001 (reconfirmation template not yet implemented), NF-002 (`INVITEE_NO_SHOW` does not exist in `WorkflowTriggerEvents`), NF-003 ("Send from my email" not yet implemented).

### Cross-Domain Integration Validation — ⬜ Pending

The following integration scenarios require validation:

- Routing form submissions correctly trigger `FORM_SUBMITTED` webhook events (S4 ↔ S5)
- Routing form embeds work correctly with enhanced embed suite (S5 → S6)
- Webhook events fire correctly for team-scoped subscriptions with updated admin governance (S7 ↔ S4)
- Notification triggers coordinate with webhook event delivery (S4 + S7 → S8)
...
<CardGroup cols={2}>
  ...
  <Card title="Webhooks & Events" icon="webhook" href="/gap-report/webhooks-events">
    21 trigger events, versioned payloads, signature verification, retry policy, Tasker async delivery
  </Card>
