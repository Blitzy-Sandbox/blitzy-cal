...
| **Webhook notifications** | `routing_form_submission.created` (organization-scoped) | Webhook triggers on form submission via booking lifecycle events | **Medium** | Cal.com does not have a dedicated `routing_form_submission.created` webhook event; form submissions are tracked via booking webhooks. See [Webhooks & Events gap analysis](/gap-report/webhooks-events) |
...
|--------|------------|----------|---------------|-------------------|----------------|
| **RF-GAP-001** | No dedicated `routing_form_submission.created` webhook event | **Medium** | `packages/features/webhooks/` | `routing_form_submission.created` at [developer.calendly.com](https://developer.calendly.com) | Add a `FORM_SUBMITTED` webhook trigger event that fires on every routing form submission, including submissions that do not result in a booking. Cross-reference: [Webhooks & Events gap analysis](/gap-report/webhooks-events) |
| **RF-GAP-002** | No built-in routing form response analytics dashboard | **Medium** | `packages/features/routing-forms/` | [Calendly Help — Manage Routing Forms](https://help.calendly.com/hc/en-us/articles/26849420421143) | Build a response analytics view with date filtering, destination result filtering, and CSV export. Leverage existing `RoutingFormResponseRepository` data |
| **RF-GAP-003** | No third-party marketing form import (HubSpot, Marketo, Pardot) | **Medium** | `packages/app-store/routing-forms/` | [Calendly Help — HubSpot forms](https://help.calendly.com/hc/en-us/articles/12131517924119) | Create integration adapters that import form field definitions from HubSpot, Marketo, and Pardot, mapping them to Cal.com routing form fields |
| **RF-GAP-004** | No data enrichment integration (Clearbit/ZoomInfo) | **Medium** | `packages/app-store/routing-forms/` | [Calendly Routing Features](https://calendly.com/features/routing) | Integrate data enrichment providers to populate hidden routing fields with industry, company size, and other enriched data |
| **RF-ADV-001** | **Cal.com advantage:** RAQB-based `jsonLogic` rule engine | **Low** | `packages/app-store/routing-forms/lib/processRoute.tsx` | N/A | Cal.com's RAQB engine supports arbitrarily complex conditional rules with nested groups, multiple operators, and compound logic — far exceeding Calendly's simple answer-based matching |
| **RF-ADV-002** | **Cal.com advantage:** Attribute-based team member routing | **Low** | `packages/features/routing-forms/lib/findTeamMembersMatchingAttributeLogic.ts` | N/A | Cal.com matches team members based on organizational attributes (department, skill, region), enabling intelligent round-robin distribution that Calendly lacks |
| **RF-ADV-003** | **Cal.com advantage:** Native nested router support | **Low** | `packages/app-store/routing-forms/zod.ts` | N/A | Cal.com supports native nested routing forms (routers) that expand inline during evaluation, eliminating the need for Calendly's workaround of daisy-chaining separate forms |
| **RF-ADV-004** | **Cal.com advantage:** Programmatic slot calculation API | **Low** | `apps/api/v2/src/modules/routing-forms/controllers/routing-forms.controller.ts` | N/A | Cal.com's `POST /v2/routing-forms/:id/calculate-slots` endpoint enables headless scheduling workflows; Calendly provides only read-only form and submission listing |
| **RF-ADV-005** | **Cal.com advantage:** Routing trace observability | **Low** | `packages/features/routing-forms/lib/getRoutedUrl.ts` | N/A | Cal.com provides full routing decision tracing via `RoutingFormTraceService` for debugging and auditing routing logic |
| **RF-ADV-006** | **Cal.com advantage:** Fallback attribute logic | **Low** | `packages/app-store/routing-forms/zod.ts` | N/A | Cal.com supports separate `fallbackAttributesQueryValue` and `fallbackAction` when primary attribute matching yields no results |
...
<Steps>
  <Step title="Add FORM_SUBMITTED Webhook Trigger Event (RF-GAP-001)">
    Add a dedicated `FORM_SUBMITTED` (or `ROUTING_FORM_SUBMISSION_CREATED`) webhook trigger event to the `WebhookTriggerEvents` Prisma enum. This event should fire on every routing form submission, including submissions that do not result in a booking. The webhook payload should include the form ID, submission ID, questions and answers, routing destination, matched route ID, and timestamp.

    **Implementation module:** `packages/features/webhooks/` — extend `WebhookTriggerEvents` enum and add payload builder in `PayloadBuilderFactory`.

    **Validation:** Verify the webhook fires for all form submissions regardless of booking outcome.
  </Step>
  <Step title="Build Response Analytics Dashboard (RF-GAP-002)">
    Create a routing form response analytics view that provides:
    - Response listing with date, destination result, questions, and answers
    - Filtering by result type (event type redirect, custom message, external URL), event type destination, and date range
    ...
    **Validation:** Verify analytics view matches Calendly's response management capabilities.
  </Step>
  <Step title="Integrate Third-Party Marketing Forms (RF-GAP-003)">
    Create integration adapters that import form field definitions from HubSpot, Marketo, and Pardot marketing automation platforms. The adapters should:
    ...
    **Validation:** Verify imported forms trigger routing logic correctly with real CRM form submissions.
  </Step>
  <Step title="Add Data Enrichment Integration (RF-GAP-004)">
    Integrate data enrichment providers (Clearbit, ZoomInfo, or equivalent) to populate routing form fields with enriched prospect data:
    ...
    **Validation:** Verify enrichment data is available for routing decisions without requiring the visitor to manually enter it.
  </Step>
</Steps>
...
|-----------|------------|-------------------|
| **AC-RF-001** | A `FORM_SUBMITTED` webhook fires on every routing form submission, including non-booking submissions | Submit a routing form that routes to a custom message (no booking). Verify webhook payload is received with form ID, submission data, and routing destination. |
| **AC-RF-002** | Routing form response analytics dashboard displays submissions with date, destination, questions, and answers | Submit 10+ routing form responses with varying destinations. Verify analytics view shows all responses with correct filtering by result type and date range. |
| **AC-RF-003** | CSV export of routing form responses includes all submission data | Export analytics data as CSV. Verify file contains all fields: submission date, destination result, question labels, and answer values. |
| **AC-RF-004** | Third-party form import maps HubSpot form fields to Cal.com routing form fields | Connect a HubSpot form with 5+ fields. Verify all field types (text, dropdown, checkbox) are correctly mapped and routing logic evaluates imported submissions. |
| **AC-RF-005** | Data enrichment populates routing fields without visitor input | Submit a form with only an email address. Verify enrichment data (company size, industry) is used in routing decisions. |

### Regression Test Scenarios

| Test Scenario | Expected Behavior |
|--------------|-------------------|
| **RT-RF-001**: Submit form matching first route condition | Route evaluates via `findMatchingRoute`, first matching route is selected, redirect to configured event type |
| **RT-RF-002**: Submit form matching no route conditions | Fallback route is selected, configured fallback action (message/redirect) is executed |
| **RT-RF-003**: Submit form with attribute-based route targeting team event | `findTeamMembersMatchingAttributeLogic` evaluates attributes, matching team member IDs are forwarded to booking page |
| **RT-RF-004**: Submit form with CRM lookup for round-robin event | `routerGetCrmContactOwnerEmail` retrieves contact owner from Salesforce/HubSpot, owner is prioritized in round-robin assignment |
| **RT-RF-005**: Submit form through nested router (multi-step) | Nested router routes are expanded inline, evaluation proceeds through all expanded routes, correct destination is reached |
| **RT-RF-006**: Submit form via API v2 `calculate-slots` endpoint | Slot calculation returns routed event type and available time slots without recording a form response |
| **RT-RF-007**: Submit duplicate form within rate limit window | Rate limiter blocks duplicate submission, returns appropriate error |
| **RT-RF-008**: Submit form with `cal.isBookingDryRun=true` | Form evaluation proceeds normally but no response is recorded and no routing trace is saved |
