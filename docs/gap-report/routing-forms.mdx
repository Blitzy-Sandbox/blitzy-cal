...
| `radio` | Radio button single-select | Route on selected option |
| `checkbox` | Checkbox multi-select | Route on selected options |

Each field has properties: `id`, `label`, `identifier`, `placeholder`, `type`, `required`, `deleted`, `options` (array of `{ label, id }` for select/multiselect/radio/checkbox fields), and optional `fieldType`, `validation`, `defaultValue`, and `description` extensions.
