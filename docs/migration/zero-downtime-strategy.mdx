...
Each migration is a timestamped directory within `packages/prisma/migrations/` containing a `migration.sql` file with the raw SQL statements. The `migration_lock.toml` file anchors the migration system to the PostgreSQL provider. With 585 migrations spanning from June 2021 to February 2026, Cal.com has extensive precedent for safe, additive schema evolution.
...
The following patterns are **proven safe** across Cal.com's 585 existing migrations. Every gap closure migration must use one or more of these patterns. Each pattern includes a real SQL example from the codebase, an explanation of why it maintains zero-downtime safety, and a timing estimate.
