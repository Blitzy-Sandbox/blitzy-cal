...
| Epic ID | Title | Priority | Complexity | Dependencies | Cal.com Package |
|---------|-------|----------|------------|-------------|----------------|
| **ET-001** | 1:1 event type behavioral parity | Medium | M | AV-001 | `packages/features/eventtypes/` |
| **ET-002** | Group event type parity (seatsPerTimeSlot) | Medium | M | ET-001 | `packages/features/eventtypes/` |
| **ET-003** | Round-robin distribution parity | High | L | ET-001 | `packages/features/eventtypes/` |
| **ET-004** | Collective scheduling parity | Medium | M | ET-001 | `packages/features/eventtypes/` |
| **ET-005** | Booking window configuration alignment | Medium | S | AV-005 | `packages/features/eventtypes/` |
| **ET-006** | Custom fields/questions parity | Low | M | ET-001 | `packages/features/eventtypes/` |
...
| **CI-005** | Bi-directional sync verification | High | L | CI-001, CI-002 | `packages/features/calendars/` | 🔄 In Progress |
...
<Info>
  **Cal.com Advantages:** Cal.com provides **11+ calendar adapters** vs. Calendly's **3 native integrations** (Google, Outlook, iCloud). Additional adapters include CalDAV, Exchange 2013/2016, Lark, Feishu, Zoho, and ICS Feed. Cal.com also provides **AES-256 credential encryption** for all stored OAuth tokens and calendar credentials, per-event-type calendar selection, and multi-calendar busy time aggregation.
  ...
  Calendar Integration epics CI-001 through CI-004 have been completed in Sprint 3. CI-005 (Bi-directional sync verification) is in progress — dedicated verification tests are pending creation. Two additional gap closures (calendar-driven cancellation sync and buffer time visualization) have integration points ready (schema, DI tokens, feature flags) with service implementation pending. Gate 3 validation is in progress.
</Info>
...
| Epic ID | Title | Priority | Complexity | Dependencies | Cal.com Package |
|---------|-------|----------|------------|-------------|----------------|
| **WH-001** | Webhook event mapping for Calendly `invitee.created` equivalents | High | M | ET-001 | `packages/features/webhooks/` |
| **WH-002** | Webhook event mapping for Calendly `invitee.canceled` equivalents | High | M | WH-001 | `packages/features/webhooks/` |
| **WH-003** | Routing form submission webhook parity (`routing_form_submission.created`) | Medium | M | RF-001 | `packages/features/webhooks/` |
| **WH-004** | Payload structure alignment with Calendly expectations | High | L | WH-001 | `packages/features/webhooks/lib/factory/` |
| **WH-005** | Webhook versioning strategy for gap closure additions | Medium | L | WH-004 | `packages/features/webhooks/lib/factory/versioned/` |
...
| Epic ID | Title | Priority | Complexity | Dependencies | Cal.com Package |
|---------|-------|----------|------------|-------------|----------------|
| **RF-001** | Routing form builder behavioral parity | Medium | M | ET-001 | `packages/features/routing-forms/` |
| **RF-002** | Conditional routing logic alignment | Medium | L | RF-001 | `packages/app-store/routing-forms/lib/processRoute.tsx` |
| **RF-003** | Form field type parity | Low | S | RF-001 | `packages/features/routing-forms/` |
| **RF-004** | Routing form API v2 endpoint parity | Medium | M | RF-001 | `apps/api/v2/src/modules/routing-forms/` |
...
| Epic ID | Title | Priority | Complexity | Dependencies | Cal.com Package |
|---------|-------|----------|------------|-------------|----------------|
| **EM-001** | Inline embed behavioral parity | Medium | M | ET-001 | `packages/embeds/embed-core/` |
| **EM-002** | Modal/popup embed parity | Medium | M | EM-001 | `packages/embeds/embed-core/` |
| **EM-003** | Floating button embed parity | Low | S | EM-001 | `packages/embeds/embed-core/` |
| **EM-004** | Share flow and link generation parity | Medium | M | ET-001 | `packages/embeds/` |
...
| Epic ID | Title | Priority | Complexity | Dependencies | Cal.com Package |
|---------|-------|----------|------------|-------------|----------------|
| **AG-001** | Admin role model parity with Calendly admin/owner/user | Medium | M | None | `packages/features/ee/organizations/` |
| **AG-002** | Team event routing behavioral parity | High | L | AG-001, ET-003 | `packages/features/ee/teams/` |
| **AG-003** | Managed event type push behavior parity | Medium | M | AG-001, ET-001 | `packages/features/eventtypes/` |
| **AG-004** | Member invitation workflow parity | Low | S | AG-001 | `packages/features/membership/` |
...
| Epic ID | Title | Priority | Complexity | Dependencies | Cal.com Package |
|---------|-------|----------|------------|-------------|----------------|
| **NF-001** | Booking confirmation email parity | High | M | ET-001 | `packages/emails/` |
| **NF-002** | Reminder email/SMS parity with Calendly | High | M | NF-001 | `packages/emails/`, `packages/sms/` |
| **NF-003** | Follow-up notification parity | Medium | M | NF-001 | `packages/emails/` |
| **NF-004** | Cancellation notification parity | High | M | NF-001 | `packages/emails/` |
| **NF-005** | SMS reminder configuration parity | Medium | M | NF-002 | `packages/sms/sms-manager.ts` |
...
| Domain | Total Epics | Critical | High | Medium | Low |
|--------|------------|----------|------|--------|-----|
| Availability & Scheduling | 7 | 0 | 1 | 5 | 1 |
| Event Types | 6 | 0 | 1 | 4 | 1 |
| Calendar Integrations | 5 | 0 | 2 | 3 | 0 |
| Webhooks & Events | 5 | 0 | 3 | 2 | 0 |
| Routing Forms | 4 | 0 | 0 | 3 | 1 |
| Embed & Share | 4 | 0 | 0 | 3 | 1 |
| Admin & Teams | 4 | 0 | 1 | 2 | 1 |
| Notifications | 5 | 0 | 3 | 2 | 0 |
| **Total** | **40** | **0** | **11** | **24** | **5** |

<Note>
  **Sprint 3 In Progress:** Calendar Integration epics CI-001 through CI-004 have been completed. CI-005 (Bi-directional Sync) is in progress with dedicated verification tests pending creation. Gate 3 validation is in progress — final validation evidence will be recorded after all implementation and tests are complete.
</Note>

### Priority Distribution

- **Critical (0 epics):** No blocking gaps exist — Cal.com does not completely lack any core Calendly capability.
- **High (11 epics):** These represent significant behavioral differences requiring attention in early sprints. Key areas include DST handling (AV-006), round-robin distribution (ET-003), conflict detection (CI-004), bi-directional sync (CI-005), webhook event mapping (WH-001, WH-002), payload alignment (WH-004), team event routing (AG-002), and notification parity (NF-001, NF-002, NF-004).
...
<Info>
  **Sprint 3 Status:** Calendar Integration epics CI-001 through CI-004 are complete. CI-005 and gap closure services are in progress. Gate 3 validation is pending — Sprint 4 (Webhooks & Events) remains blocked until Gate 3 is formally passed with all five validation dimensions verified.
</Info>
