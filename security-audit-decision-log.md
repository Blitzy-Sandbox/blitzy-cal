...
A read-only, five-layer **Full Security Stack** audit of the Cal.com monorepo (`calcom-monorepo`, a Yarn Berry / Node.js 20 workspace). The engagement is **detection-and-reporting only**: it reads the application source tree exhaustively but modifies **~0 application files**. Its only outputs are the net-new audit artifacts (the four normalized per-layer finding files, the four Layer 3a inventories, the two raw scanner outputs, and the cross-layer merged report) plus the two rule-mandated deliverables — this decision log and the sibling reveal.js executive presentation. All findings are normalized to a single **unified severity vocabulary** (`critical` / `high` / `medium` / `low`), and the pipeline follows a strict **deterministic-before-agent** methodology so that machine-generated ground truth anchors and bounds every agent-reasoning step.
...
The audit merged **four** measurement layers under one severity schema and produced a single cross-correlated report whose CI/CD gate verdict is **`BLOCK`**. The headline figures below are the canonical numbers emitted by `findings-merged.json._summary` (generated `2026-06-05T00:38:14Z`); they are reproduced unchanged in the traceability matrix (§5), the verification suite (§6), and the executive presentation, and any figure appearing in this log equals its counterpart there (§10).
...
| Cross-layer corroborated findings | **84** (28 high-confidence by shared file, 56 medium-confidence by shared CWE) | `corroborated_count`, `corroboratedByConfidence` |
| Corroborated files / CWE themes | 12 files / 7 CWE themes | `corroboratedFiles`, `corroboratedCwes` |
...
5. **Stage E — Governance deliverables.** This decision log and the executive presentation.
...
| **Cross-layer corroboration method.** Annotate findings confirmed by more than one layer, matching on shared source file (high confidence) and shared CWE (medium confidence); only cross-layer matches count. | Report each layer in isolation with no correlation; or correlate by free-text similarity. | Corroboration elevates issues seen independently by multiple layers, focusing attention where evidence converges (AAP §0.6.2, Directive 8). This run corroborated 84 findings (28 high-confidence across 12 files, 56 medium-confidence across 7 CWE themes). | Thematic CWE matches can over-group unrelated issues; mitigated by labeling confidence (`high` for shared file, `medium` for shared CWE) so consumers can weight accordingly. |
...
| D8 | Cross-layer merged report with `_summary` header and corroboration annotations | `findings-merged.json` | Done — 298 findings, 84 corroborated |
...
| `security-audit-executive-presentation.html` | AAP §0.8.2 (Executive Presentation rule) | No |

Every directive maps to at least one artifact and every artifact maps back to a directive or rule, so coverage is complete in both directions.
...
| C4 | Merged `totalFindings` equals the sum of the four per-layer counts | Count integrity | **PASS** — 29 + 32 + 65 + 172 = 298 |
| C5 | Merged `bySeverity` sums to `totalFindings` | Severity-count integrity | **PASS** — 18 + 96 + 110 + 74 = 298 |
...
- **Decision.** Compute and document the `BLOCK` verdict, but do not edit `.github/workflows/security-audit.yml` or `all-checks.yml` to enforce it within this engagement.
...
- **Compensating controls.** The verdict, its precedence model (§2.3), the per-layer triggering counts (§1), and the exact umbrella integration point (`all-checks.yml`'s `always()` required-job gate) are documented so enforcement is a small, well-scoped change.
...
- **Sibling deliverable produced separately.** The executive presentation (`security-audit-executive-presentation.html`) is a distinct deliverable that reuses the canonical figures recorded here; this log does not author it, but its figures must match (§10).
...
The integrity checks in §6 are deterministic functions of the committed artifacts. The core count checks (C4–C7, C15) recompute directly from the merged report:

```bash
python3 - <<'PY'
import json
s = json.load(open("findings-merged.json"))["_summary"]
per = {k: v["findings"] for k, v in s["layerStatus"].items()}
assert s["totalFindings"] == sum(per.values()) == 298                  # C4
assert sum(s["bySeverity"].values()) == 298                            # C5
...
Every quantitative figure in this decision log is drawn from `findings-merged.json._summary` and **equals** its counterpart in that file and in the sibling executive presentation (`security-audit-executive-presentation.html`). The merged report itself states that these are "the canonical numbers reused by `security-audit-decision-log.md` and `security-audit-executive-presentation.html`." The shared canonical figures are:
...
| Corroborated findings (high / medium confidence) | 84 (28 / 56) |
| Gate verdict | `BLOCK` |
