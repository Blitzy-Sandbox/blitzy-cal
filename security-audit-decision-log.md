...
| **Layer 3a inventory scan scope = `apps` + `packages`.** Enumerate sinks and mitigations only under the two workspaces that hold the production application source the audit targets, then partition matches into production (`*-inventory.txt`) vs. test (`*-inventory-test.txt`). Root-level operational/seed scripts (`scripts/`), the Vitest setup shim (`setupVitest.ts`), and the demonstration workspaces (`example-apps/`) are outside the inventory scan scope. | Scan the whole repository tree (every workspace plus root-level `scripts/` and `example-apps/`); or leave those support/demo files in the production inventory; or route them into the `*-test.txt` partition. | The Layer 3a in-scope read targets are the `apps/**` and `packages/**` application source (AAP §0.3.1, §0.7.1); seed/operational scripts, the test-runner setup shim, and example apps are neither production application code nor test fixtures of it, so including them would skew the production inventory and the per-category sink counts that bound Layer 3b. Scoping to `apps`/`packages` makes the exact reproduction command in §9.2 (`find apps packages …`) regenerate the precise root set of every inventory, and every Layer 3b `inventoryRef` (all under `apps/`/`packages/`) stays grounded (check C12). | A genuine sink confined to a root-level script or an example app is not inventoried by Layer 3a, hence not taint-analyzed by Layer 3b; mitigated because Layer 1 architectural review and Layer 2 Semgrep still cover those regions and such files are non-production. |
...
| **Executive-deck content budget** *(strict ≤40-word interpretation, revised after QA)*. Apply the ≤40-word content-slide limit (AAP §0.8.2) to **every rendered word on a default content slide, including data-table header and cell text**; only the Mermaid diagram SVG/source is excluded as the slide's non-text visual. The two data-table slides were reduced to satisfy this strict count: slide 08 now lists the four **severity totals** (Critical 18, High 96, Medium 110, Low 74; All 298) and slide 13 a compact **risk → existing-defense** pairing. Each table's accessible name is supplied with an `aria-label` **attribute** (never part of rendered text or `textContent`) instead of an `sr-only` `<caption>` (whose text a `textContent` word-count includes); `scope="col"` headers remain and Lucide icons stay decorative (`aria-hidden="true"`). | Keep the fuller by-layer-by-severity matrix and 3-column risk table and argue cell text is exempt non-prose; or split each dense table across multiple slides; or drop the tables (the AAP mandates a `data-table`, so dropping is disallowed). | A QA reviewer's word count is taken over visible/`textContent` text and includes table cells and any `sr-only` caption, so the robust fix is to make the slides **genuinely** ≤40 words rather than rely on a prose-only reading; the severity totals are the headline numbers and the risk→defense pairs preserve the executive message. Counted the way the QA checkpoint does — every rendered alphanumeric word, table header and cell text included, with Mermaid SVG, standalone separator glyphs, and ARIA attributes excluded — every content slide is ≤40 words (maximum 36; the two former offenders are now 23 and 35). | The deck no longer shows the full per-layer-by-severity breakdown; mitigated by retaining the per-layer totals (29 · 32 · 65 · 172) in the slide-08 source-note, the headline KPIs on slides 03 and 10, and the complete matrix in `findings-merged.json`. |
| **Executive-deck keyboard focus and mobile readability** *(added after QA)*. Add a brand-token `:focus-visible` ring (teal outline + purple halo) to the reveal navigation controls, the pause-overlay resume button, and in-slide links, and add `@media (max-width:768px)` / `(max-width:480px)` rules that raise the `data-table` font and padding so the severity and risk tables stay legible after reveal scales its fixed 1920×1080 canvas down on small viewports. | Rely on the browser's default focus outline (reveal's theme removes it, leaving no visible indicator); leave the canvas-scaled tables tiny on phones; or manage focus with JavaScript. | WCAG 2.4.7 requires a visible focus indicator for keyboard users, and reveal's fixed-canvas down-scaling renders table text only a few pixels tall at 375px; a brand-token ring and a relative-rem mobile type scale fix both without altering the reveal/Mermaid/Lucide runtime config or the brand palette. | The exact ring thickness and mobile font sizes are heuristic; mitigated by runtime verification — `getComputedStyle` confirms a non-`none` outline and box-shadow on the focused control, and a 375px screenshot confirms the table is larger with no overflow. |
...
**Scan roots and exclusions.** The two enumeration roots are `apps` and `packages` — the workspaces that hold the production application source this audit targets (AAP §0.3.1, §0.7.1). Root-level operational/seed scripts (`scripts/`), the repository's Vitest setup shim (`setupVitest.ts`), and the demonstration workspaces under `example-apps/` are intentionally outside this Layer 3a scan scope; they are neither production application code nor its test fixtures. Consequently every line in all four inventories begins with `apps/` or `packages/`, so the `find apps packages …` command above reproduces the exact root set of each inventory (production and test alike). The per-category sink counts in `findings-layer-3b-blitzy-taint.json` (`coverage[].sinksInInventory`, summed as `summary.totalSinksInInventory`) are computed against this `apps`/`packages` production sink inventory.
