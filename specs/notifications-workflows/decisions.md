...
- The workflow engine in `packages/features/ee/workflows/` is not modified — in-app notifications triggered by workflows are created by the `NotificationService` as a downstream consumer of workflow trigger events, not as a workflow action type
